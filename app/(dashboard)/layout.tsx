...
} from "@/config/session-security";

const AICopilotSidebar = dynamic(
  () =>
    import("@/components/ai-copilot-sidebar").then(
      (mod) => mod.AICopilotSidebar,
    ),
  {
    ssr: false,
...
function DashboardLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const isVestaFullscreen =
    vestaShellUI.vestaViewMode === "fullscreen" && !sidebarState.rightCollapsed;

  // Keyboard shortcuts
  // Cmd+B for left sidebar
  useKeyboardShortcut({ key: "b", metaKey: true }, toggleLeftSidebar, []);

  // Cmd+/ for right sidebar
  useKeyboardShortcut({ key: "/", metaKey: true }, toggleRightSidebar, []);
  ...
  useSessionIdleGuard({
    ...
    onWarning: () => {
      const remainingMinutes = Math.max(
        1,
        Math.floor(SESSION_WARNING_LEAD_MS / 60000),
      );
      toast.warning(
