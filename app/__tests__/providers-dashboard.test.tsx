...
describe("DashboardProviders", () => {
  ...
  it("keeps dashboard runtime enabled for legacy dashboard aliases before redirect", () => {
    pathname = "/vesta";
    render(
      <DashboardProviders>
        <div>legacy route child</div>
      </DashboardProviders>,
    );

    expect(screen.getByTestId("dashboard-runtime")).toBeInTheDocument();
    expect(screen.getByText("legacy route child")).toBeInTheDocument();
  });
});
