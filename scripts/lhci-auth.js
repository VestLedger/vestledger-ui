...
function attachDiagnostics(page) {
  ...
  page.on("console", (msg) => {
    ...
    const locationSuffix =
      location && location.url
        ...
        : "";
    console.log(
      `${LOG_PREFIX} console.${msg.type()}: ${msg.text()}${locationSuffix}`,
    );
  });
  ...
  page.on("requestfailed", (request) => {
    ...
    console.warn(
      `${LOG_PREFIX} requestfailed: ${request.method()} ${request.url()} ${failure ? failure.errorText : ""}`,
    );
  });
  ...
  page.on("response", (response) => {
    if (response.status() >= 400) {
      console.warn(
        `${LOG_PREFIX} response ${response.status()}: ${response.url()}`,
      );
    }
  });
}
