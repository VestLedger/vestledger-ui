...
function buildHostForType(
  hostname: string,
  port: string,
  hostType: Exclude<HostType, "localhost">,
): string {
  const baseHostname = resolveBaseHostname(hostname);
  const publicHostname =
    process.env.NEXT_PUBLIC_USE_WWW === "true"
      ? `www.${baseHostname}`
      : baseHostname;
...
function isPublicRoute(pathname: string): boolean {
  return (
    ...
    PUBLIC_MARKETING_ROUTES.filter(
      (route) => route !== ACCESS_ROUTE_PATHS.publicHome,
...
function isPhoneUserAgent(userAgent: string): boolean {
  ...
  return /iphone|ipod|android.*mobile|windows phone|blackberry|opera mini|mobile/.test(
    ua,
  );
}
...
export function middleware(request: NextRequest) {
  ...
  const configuredPublicHost = normalizeConfiguredHost(
    process.env.NEXT_PUBLIC_PUBLIC_DOMAIN,
  );
  const configuredAppHost = normalizeConfiguredHost(
    process.env.NEXT_PUBLIC_APP_DOMAIN,
  );
  const configuredAdminHost = normalizeConfiguredHost(
  ...
  const matchesProtectedRoute =
    matchesDashboardRoute || matchesAdminRoute || isVestaRoute;
  ...
  if (hostType === "localhost") {
    ...
    if (
      !isAuthenticated &&
      normalizedPathname === ACCESS_ROUTE_PATHS.publicHome
    ) {
    ...
    if (
      ...
      const nextPath =
        target === "admin" ? ADMIN_DEFAULT_PATH : APP_DEFAULT_PATH;
    ...
    if (
      ...
      !canRoleAccessPath(currentUser.role, normalizedPathname)
    ) {
  ...
  if (
    (domainTarget === "admin" && hostType !== "admin") ||
    (domainTarget === "app" && hostType !== "app")
  ) {
    const targetPath =
      domainTarget === "admin"
        ? matchesAdminRoute
  ...
  if (isLoginRoute) {
    return redirectToHost(
      url,
      targetHost,
      domainTarget === "admin" ? ADMIN_DEFAULT_PATH : APP_DEFAULT_PATH,
    );
  }
