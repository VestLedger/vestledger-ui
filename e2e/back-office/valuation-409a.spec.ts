...
test.describe("409A Valuations - Tabs Navigation", () => {
  ...
  test("should switch to Strike Prices tab", async ({ page }) => {
    ...
    const strikePriceContent = page.locator(
      "text=/strike price|option|grant/i",
    );
...
test.describe("409A Valuations - Valuation List", () => {
  ...
  test("should show company name on valuation cards", async ({ page }) => {
    ...
    const companyName = page
      ...
      .filter({ hasText: /company|inc|corp|llc/i });
  ...
  test("should show status badge on valuation cards", async ({ page }) => {
    ...
    const statusBadge = page
      ...
      .filter({ hasText: /current|expiring|expired/i });
...
test.describe("409A Valuations - Interactions - Data Verification", () => {
  ...
  test("switching to history tab should show historical data", async ({
    page,
  }) => {
    const valuation = new Valuation409APage(page);
    await valuation.goto();
    ...
    const changed = verifyDataChanged(
      valuationsTabSnapshot,
      historyTabSnapshot,
    );
  ...
  test("company filter should update valuation list", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="valuation-item"], table tbody tr';
    ...
    if (await companyFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await option.isVisible()) {
          ...
          expect(changed, "Company filter should update valuation list").toBe(
            true,
          );
        }
      }
  ...
  test("status filter should update displayed valuations", async ({ page }) => {
    ...
    const statusFilter = page
      ...
      .or(
        page
          .locator("select")
          .filter({ hasText: /status|all status|current|expiring/i }),
      );
  ...
  test("clicking valuation card should show details", async ({ page }) => {
    ...
    const valuationCards = page
      .locator('[data-testid="valuation-card"], div.rounded-lg')
      .filter({ hasText: /valuation|FMV|\$/i });
