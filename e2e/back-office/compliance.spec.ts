...
test.describe("Compliance - Status Filtering", () => {
  test("should filter by overdue status", async ({ page }) => {
    ...
    if (await compliance.statusFilter.isVisible()) {
      ...
      const overdueItems =
        await compliance.getComplianceItemsByStatus("overdue");
  ...
  test("should filter by in-progress status", async ({ page }) => {
    ...
    if (await compliance.statusFilter.isVisible()) {
      ...
      const inProgressItems =
        await compliance.getComplianceItemsByStatus("in-progress");
  ...
  test("should filter by completed status", async ({ page }) => {
    ...
    if (await compliance.statusFilter.isVisible()) {
      ...
      const completedItems =
        await compliance.getComplianceItemsByStatus("completed");
...
test.describe("Compliance - Priority Filtering", () => {
  test("should filter by high priority", async ({ page }) => {
    ...
    if (await compliance.priorityFilter.isVisible()) {
      ...
      const highPriorityBadges = page.locator(
        '[class*="danger"], [class*="high"]',
      );
...
test.describe("Compliance - Interactions - Data Verification", () => {
  test("status filter should update compliance items list", async ({
    page,
  }) => {
    const compliance = new CompliancePage(page);
    await compliance.goto();
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="compliance-item"], table tbody tr';
  ...
  test("priority filter should update compliance items list", async ({
    page,
  }) => {
    const compliance = new CompliancePage(page);
    await compliance.goto();
  ...
  test("switching between tabs should show different content", async ({
    page,
  }) => {
    const compliance = new CompliancePage(page);
    await compliance.goto();
    ...
    const dataSelector =
      'div.rounded-lg, [class*="content"], table, [class*="panel"]';
  ...
  test("combined status and priority filters should work together", async ({
    page,
  }) => {
    const compliance = new CompliancePage(page);
    await compliance.goto();
    ...
    if (initialSnapshot.count > 2) {
      ...
      expect(
        filtersApplied,
        "Combined filters should affect compliance items",
      ).toBe(true);
    }
  });
});
