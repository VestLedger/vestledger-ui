...
test.describe("Pipeline", () => {
  ...
  test("should display pipeline content", async ({ page }) => {
    ...
    const content = page.locator(
      '[data-testid="pipeline"], [class*="pipeline"], table, [class*="kanban"]',
    );
  ...
  test("should have stage management", async ({ page }) => {
    ...
    const stages = page.locator(
      '[data-testid="stage"], [class*="stage"], [class*="column"]',
    );
...
test.describe("Contacts", () => {
  ...
  test("should display contacts list", async ({ page }) => {
    ...
    const content = page.locator(
      'table, [data-testid="contacts"], [class*="contact"]',
    );
    await expect(content.first()).toBeVisible({ timeout: 10000 });
  });
...
test.describe("Pipeline - Interactions - Data Verification", () => {
  test("stage filter should update pipeline view", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="deal"], table tbody tr, [class*="column"]';
    ...
    if (await stageFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await option.isVisible()) {
          ...
          expect(changed, "Stage filter should update pipeline view").toBe(
            true,
          );
        }
      }
  ...
  test("view toggle should change pipeline display", async ({ page }) => {
    ...
    const viewToggle = page
      .getByRole("button", { name: /list|grid|kanban|table/i })
      .or(page.locator('[data-testid="view-toggle"]'));

    const dataSelector =
      'div.rounded-lg, [class*="column"], table, [class*="kanban"], [class*="list"]';
    ...
    if (await viewToggle.first().isVisible()) {
      ...
      expect(changed, "View toggle should change pipeline display").toBe(true);
    }
  });
    ...
    if (await statusFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await option.isVisible()) {
          ...
          expect(changed, "Status filter should filter deals").toBe(true);
        }
      }
    }
  });
  ...
  test("clicking deal should show deal details", async ({ page }) => {
    ...
    if ((await dealCards.count()) > 0) {
      ...
      const urlChanged =
        page.url() !== "http://localhost:3000/pipeline" &&
