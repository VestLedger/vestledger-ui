...
test.describe("Integrations - Page Load", () => {
  ...
  test("should display integrations content", async ({ page }) => {
    ...
    const content = page.locator(
      'div.rounded-lg, [data-testid="integration"], [class*="integration"]',
    );
...
test.describe("Integrations - Available Integrations", () => {
  test("should display available integration cards", async ({ page }) => {
    ...
    const integrationCards = page.locator(
      '[data-testid="integration-card"], div.rounded-lg',
    );
  ...
  test("should show integration names", async ({ page }) => {
    ...
    const integrationNames = page.locator(
      "text=/quickbooks|salesforce|google|dropbox|slack|plaid|stripe|xero/i",
    );
  ...
  test("should show integration status badges", async ({ page }) => {
    ...
    const statusBadge = page
      .locator('[class*="badge"]')
      .filter({ hasText: /connected|disconnected|available|active/i });
...
test.describe("Integrations - Connection Actions", () => {
  test("should have connect button for available integrations", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/integrations");
    await page.waitForLoadState("networkidle");
  ...
  test("should have disconnect button for connected integrations", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/integrations");
    await page.waitForLoadState("networkidle");
    ...
    const disconnectButton = page.getByRole("button", {
      name: /disconnect|remove|disable/i,
    });
  ...
  test("should have configure/settings option for connected integrations", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/integrations");
    await page.waitForLoadState("networkidle");
    ...
    const configureButton = page.getByRole("button", {
      name: /configure|settings|manage/i,
    });
...
test.describe("Integrations - Categories", () => {
  test("should display integration categories", async ({ page }) => {
    ...
    const categories = page.locator(
      "text=/accounting|crm|storage|communication|banking|calendar/i",
    );
  ...
  test("should search integrations by name", async ({ page }) => {
    ...
    if (await searchInput.isVisible()) {
      ...
      const results = page.locator(
        'div.rounded-lg, [data-testid="integration"]',
      );
...
test.describe("Integrations - Connected Integrations", () => {
  test("should show connected integrations section", async ({ page }) => {
    ...
    const connectedSection = page.locator(
      "text=/connected|active integrations/i",
    );
  ...
  test("should show last sync time for connected integrations", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/integrations");
    await page.waitForLoadState("networkidle");
...
test.describe("Integrations - API Keys", () => {
  ...
  test("should have generate API key button", async ({ page }) => {
    ...
    const generateButton = page.getByRole("button", {
      name: /generate.*key|create.*key|new.*key/i,
    });
...
test.describe("Integrations - Interactions - Data Verification", () => {
  test("category filter should update integrations list", async ({ page }) => {
    ...
    const categoryFilter = page
      ...
      .or(page.locator('[data-testid="category-filter"]'))
      .or(
        page.locator("select").filter({ hasText: /category|all categories/i }),
      );

    const dataSelector =
      'div.rounded-lg, [data-testid="integration"], [data-testid="integration-card"]';
  ...
  test("status filter should update integrations list", async ({ page }) => {
    ...
    if (await statusFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        const connectedOption = page.getByRole("option", {
          name: /connected/i,
        });
        if (await connectedOption.isVisible()) {
          ...
          expect(changed, "Status filter should update integrations list").toBe(
            true,
          );
        }
      }
  ...
  test("clicking integration card should show details", async ({ page }) => {
    ...
    const integrationCards = page.locator(
      'div.rounded-lg, [data-testid="integration-card"]',
    );

    if ((await integrationCards.count()) > 0) {
      const detailsSelector =
        '[role="dialog"], [class*="drawer"], [class*="detail"], [class*="panel"], [class*="modal"]';
  ...
  test("tab navigation should update integrations view", async ({ page }) => {
    ...
    if ((await tabs.count()) > 1) {
      ...
      expect(changed, "Tab navigation should update integrations view").toBe(
        true,
      );
    }
  });
  ...
  test("sync button should trigger data refresh", async ({ page }) => {
    ...
    const syncButton = page.getByRole("button", { name: /sync|refresh/i });
    const dataSelector =
      'div.rounded-lg, [data-testid="integration"], [class*="status"], [class*="sync"]';
