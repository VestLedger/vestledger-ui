...
test.describe("Distribution Full Flow", () => {
  test.describe("Wizard Navigation", () => {
    test("should load distribution wizard with all 9 steps visible", async ({
      page,
    }) => {
    ...
    test("should navigate forward through steps with valid data", async ({
      page,
    }) => {
      await loginViaRedirect(page, "/fund-admin/distributions/new");
      await page.waitForLoadState("networkidle");
      ...
      const grossProceedsInput = page
        .getByLabel(/gross proceeds/i)
        .or(page.locator('input[name*="gross"]'));
      ...
      const nextButton = page.getByRole("button", {
        name: /save.*continue|next|continue/i,
      });
  ...
  test.describe("Step 1: Event Details", () => {
    ...
    test("should display event type selector", async ({ page }) => {
      ...
      const eventTypeSelect = page
        ...
        .or(
          page
            .locator("select")
            .filter({ hasText: /exit|dividend|recapitalization/i }),
        )
    ...
    test("should display date pickers for event and payment dates", async ({
      page,
    }) => {
      await loginViaRedirect(page, "/fund-admin/distributions/new");
      await page.waitForLoadState("networkidle");

      const eventDateInput = page
        .getByLabel(/event date/i)
        .or(page.locator('input[type="date"]').first());
      const paymentDateInput = page
        .getByLabel(/payment date/i)
  ...
  test.describe("Step 2: Fees & Expenses", () => {
    ...
    test("should allow adding fee line items", async ({ page }) => {
      ...
      const addFeeButton = page.getByRole("button", {
        name: /add.*fee|add.*expense|add.*line/i,
      });
  ...
  test.describe("Step 3: Waterfall Selection", () => {
    test("should display available waterfall scenarios", async ({ page }) => {
      await loginViaRedirect(
        page,
        "/fund-admin/distributions/new?step=waterfall",
      );
      await page.waitForLoadState("networkidle");

      // Should show waterfall selection UI
      const waterfallSection = page
        .locator("text=/waterfall|scenario/i")
        .first();
    ...
    test("should validate waterfall scenario selection", async ({ page }) => {
      ...
      if (await nextButton.isVisible()) {
        ...
        const errorOrSameStep =
          (await page.locator('[class*="danger"], [class*="error"]').count()) >
            0 || (await page.locator("text=/Step 3|waterfall/i").isVisible());
  ...
  test.describe("Step 4: LP Allocations", () => {
    test("should display LP allocations table", async ({ page }) => {
      await loginViaRedirect(
        page,
        "/fund-admin/distributions/new?step=allocations",
      );
      await page.waitForLoadState("networkidle");

      const allocationsTable = page
        .locator('table, [role="grid"]')
        .filter({ hasText: /LP|allocation|gross|net/i });
    ...
    test("should have recalculate allocations button", async ({ page }) => {
      ...
      const recalculateButton = page.getByRole("button", {
        name: /recalculate|refresh|update/i,
      });
  ...
  test.describe("Step 5: Tax Withholding", () => {
    ...
    test("should validate tax rates are within range", async ({ page }) => {
      ...
      const taxRateInputs = page
        .locator('input[type="number"]')
        .filter({ hasText: /tax|rate|%/i });
  ...
  test.describe("Step 8: Preview", () => {
    test("should display statement template selection", async ({ page }) => {
      await loginViaRedirect(
        page,
        "/fund-admin/distributions/new?step=preview",
      );
      await page.waitForLoadState("networkidle");

      const previewSection = page
        .locator("text=/preview|template|statement/i")
        .first();
    ...
    test("should have email subject and body fields", async ({ page }) => {
      ...
      const emailSubject = page
        .getByLabel(/email subject|subject/i)
        .or(page.locator("input").filter({ hasText: /subject/i }));
      const emailBody = page
        .getByLabel(/email body|message/i)
  ...
  test.describe("Step 9: Submit for Approval", () => {
    test("should display approval workflow information", async ({ page }) => {
      ...
      const submitSection = page
        .locator("text=/submit|approval|approver/i")
        .first();
  ...
  test.describe("Running Totals Sidebar", () => {
    ...
    test("should show warnings when data is inconsistent", async ({ page }) => {
      ...
      const warningsSection = page
        .locator("div.rounded-lg")
        .filter({ hasText: /warning|reconcile|exceed/i });
  ...
  test.describe("Cancel Flow", () => {
    test("should have cancel option", async ({ page }) => {
      ...
      const cancelButton = page
        .getByRole("button", { name: /cancel|close|back to/i })
...
test.describe("Distribution Detail View", () => {
  test("should load distribution detail page", async ({ page }) => {
    ...
    const distributionItem = page
      .locator("table tbody tr, div.rounded-lg")
      .filter({ hasText: /distribution|Q\d/i })
      .first();
    ...
    if (await distributionItem.isVisible()) {
      ...
      const detailContent = page.locator(
        "text=/gross proceeds|net proceeds|allocation/i",
      );
  ...
  test("should display approval workflow on detail page", async ({ page }) => {
    ...
    const hasContent =
      (await approvalSection.first().isVisible()) ||
  ...
  test("should display LP allocation breakdown", async ({ page }) => {
    ...
    const allocationsSection = page.locator(
      "text=/LP Allocation|allocation breakdown/i",
    );
    ...
    if (await allocationsSection.first().isVisible()) {
      ...
      const allocationsTable = page
        .locator("table")
        .filter({ hasText: /LP|pro-rata|gross|net/i });
  ...
  test("should display lifecycle timeline", async ({ page }) => {
    ...
    const timeline = page.locator(
      "text=/lifecycle|timeline|draft|submitted|approved/i",
    );
...
test.describe("Distribution Approval Workflow", () => {
  test("should show approve/reject buttons for pending approval", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/fund-admin/distributions/dist-1");
    await page.waitForLoadState("networkidle");
  ...
  test("should show approval stepper with approver order", async ({ page }) => {
    ...
    const approvalStepper = page
      .locator('[class*="stepper"], [class*="approval"]')
      .filter({ hasText: /step|order|pending|approved/i });
