...
test.describe('Distribution Wizard - Waterfall Step', () => {
  test('should display Waterfall Integration heading in wizard', async ({ page }) => {
    ...
    if (await waterfallHeading.isVisible({ timeout: 5000 }).catch(() => false)) {
  ...
  test('should display waterfall scenario selector', async ({ page }) => {
    ...
    if (await waterfallHeading.isVisible().catch(() => false)) {
      ...
      const scenarioSelect = page.getByLabel(/waterfall scenario/i)
  ...
  test('should show Fund Default badge when scenario matches fund active waterfall', async ({ page }) => {
    const wizard = new DistributionWizardPage(page);
    await wizard.goto();
    await page.waitForLoadState('networkidle');
    ...
    for (let step = 0; step < 5; step++) {
      if (await wizard.waterfallIntegrationHeading.isVisible().catch(() => false)) {
        break;
      }
      if (await wizard.nextButton.isVisible()) {
        await wizard.nextButton.click();
    ...
    if (await wizard.fundDefaultBadge.isVisible({ timeout: 3000 }).catch(() => false)) {
      await expect(wizard.fundDefaultBadge).toBeVisible();
    }
  });

  test('should show Locked indicator for locked scenarios', async ({ page }) => {
    const wizard = new DistributionWizardPage(page);
    await wizard.goto();
    await page.waitForLoadState('networkidle');
  ...
  test('should hide Quick Preview for locked scenarios', async ({ page }) => {
    ...
    if (await wizard.waterfallIntegrationHeading.isVisible().catch(() => false)) {
      ...
      if (await wizard.waterfallScenarioSelect.isVisible().catch(() => false)) {
        const hasScenarioSelected = await page.locator('[class*="scenario"], [class*="border-"]').count() > 0;
        if (hasScenarioSelected && !(await wizard.lockedBadge.isVisible().catch(() => false))) {
          // Quick Preview should be visible for unlocked scenarios
          if (await wizard.quickPreviewSection.isVisible({ timeout: 3000 }).catch(() => false)) {
