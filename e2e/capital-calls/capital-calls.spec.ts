...
test.describe("Capital Calls - Fund Admin Tab", () => {
  test("should load fund admin page with capital calls tab", async ({
    page,
  }) => {
  ...
  test("should display summary metrics cards", async ({ page }) => {
    ...
    const activeCallsCard = page
      .locator("div.rounded-lg")
      .filter({ hasText: /^Active Calls$/i })
      .first();
    const outstandingCard = page
      .locator("div.rounded-lg")
      .filter({ hasText: /^Outstanding$/i })
      .first();
    const totalLPsCard = page
      .locator("div.rounded-lg")
      .filter({ hasText: /^Total LPs$/i })
      .first();

    if (
      (await activeCallsCard.count()) > 0 &&
      (await outstandingCard.count()) > 0 &&
      (await totalLPsCard.count()) > 0
    ) {
...
test.describe("Capital Calls - List View", () => {
  ...
  test("should show call number and status badge on each card", async ({
    page,
  }) => {
    await gotoCapitalCallsTab(page);
    ...
    if (await callCard.isVisible()) {
      ...
      const statusBadge = callCard.locator(
  ...
  test("should show collection progress bar for active calls", async ({
    page,
  }) => {
    await gotoCapitalCallsTab(page);
    ...
    if (await callCard.isVisible()) {
      ...
      const progressBar = callCard.locator(
      ...
      const hasProgress =
        (await progressBar.count()) > 0 || (await progressText.count()) > 0;
...
test.describe("Capital Calls - Actions", () => {
  ...
  test("should have Send Reminder button for active calls", async ({
    page,
  }) => {
    await gotoCapitalCallsTab(page);
...
test.describe("Capital Calls - LP Responses Tab", () => {
  test("should switch to LP Responses tab", async ({ page }) => {
    ...
    const lpResponsesContent = page.locator(
      "text=/LP Response|commitment|paid/i",
    );
  ...
  test("should display LP response cards with payment status", async ({
    page,
  }) => {
    const capitalCalls = new CapitalCallsPage(page);
    await capitalCalls.goto();
  ...
  test("should show payment progress for partial payments", async ({
    page,
  }) => {
    const capitalCalls = new CapitalCallsPage(page);
    await capitalCalls.goto();
    ...
    if (await lpCard.isVisible()) {
      ...
      const progressBar = lpCard.locator(
        '[role="progressbar"], [class*="progress"]',
      );
      ...
      const hasProgress =
        (await progressBar.count()) > 0 || (await paymentText.count()) > 0;
...
test.describe("Capital Calls - Tab Navigation", () => {
  ...
  test("should show tab counts/badges", async ({ page }) => {
    ...
    const tabBadges = page
      .locator('[role="tab"] [class*="badge"], [role="tab"] span')
      .filter({ hasText: /\\d+/ });
...
test.describe("Capital Calls - Interactions - Data Verification", () => {
  test("fund selector should update capital call cards", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg:has-text("Capital Call"), [data-testid="capital-call-card"]';
  ...
  test("status filter should update LP response list", async ({ page }) => {
    ...
    const statusFilter = page
      ...
      .first();
    const dataSelector =
      '[class*="rounded-lg"]:has-text("commitment"), [data-testid="lp-response-card"]';
    ...
    if (await statusFilter.isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await paidOption.isVisible()) {
          ...
          expect(changed, "Status filter should update LP response list").toBe(
            true,
          );
        }
      }
  ...
  test("tab switch should update displayed content", async ({ page }) => {
    ...
    expect(changed, "Tab switch should update displayed content").toBe(true);
  });
  ...
  test("metrics cards should reflect filtered data", async ({ page }) => {
    ...
    const metricsSelector =
      'div.rounded-lg:has-text("Active Calls"), div.rounded-lg:has-text("Outstanding"), div.rounded-lg';
