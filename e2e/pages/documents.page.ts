...
export class DocumentsPage {
  ...
  constructor(page: Page) {
    ...
    this.createFolderButton = page.getByRole("button", {
      name: /create folder|new folder/i,
    });

    // Document manager
    this.documentManager = page
      .locator('[class*="document-manager"], [class*="DocumentManager"]')
      .or(
        page
          .locator('[class*="grid"]')
          .filter({ has: page.locator("div.rounded-lg") }),
      );
    this.documentList = page.locator(
      '[class*="document-list"], [class*="grid"]',
    );
    this.documentCards = page
      .locator("div.rounded-lg")
      .filter({ has: page.locator("svg") });
    this.folderCards = page
      .locator("div.rounded-lg")
      .filter({ has: page.locator('[class*="folder" i]') });

    // Search and filters
    this.searchInput = page
      .getByPlaceholder(/search.*document/i)
      .or(page.getByRole("searchbox"));
    this.categoryFilter = page
      .getByRole("combobox", { name: /category/i })
      .or(page.locator("select").filter({ hasText: /category/i }));
    this.fundFilter = page
      .getByRole("combobox", { name: /fund/i })
      .or(page.locator("select").filter({ hasText: /fund/i }));

    // Document actions (context menu or buttons)
    this.openDocumentButton = page.getByRole("button", {
      name: /open|view|preview/i,
    });
    ...
    this.previewModal = page
      .locator('[role="dialog"]')
      .filter({ has: page.locator('[class*="preview"]') })
      .or(
        page
          .locator('[class*="modal"], [class*="Modal"]')
          .filter({ has: page.locator("iframe, img, object") }),
      );
    this.previewCloseButton = this.previewModal.getByRole("button", {
      name: /close/i,
    });
    this.previewNavigateNext = this.previewModal.getByRole("button", {
      name: /next/i,
    });
    this.previewNavigatePrev = this.previewModal.getByRole("button", {
      name: /previous|prev/i,
    });
    this.previewDownloadButton = this.previewModal.getByRole("button", {
      name: /download/i,
    });
    this.previewShareButton = this.previewModal.getByRole("button", {
      name: /share/i,
    });
  ...
  async filterByFund(fund: string) {
    if (await this.fundFilter.isVisible()) {
      ...
      await this.page
        .getByRole("option", { name: new RegExp(fund, "i") })
        .click();
      await this.page.waitForLoadState("networkidle");
    }
  }
  ...
  getDocumentsByType(type: string): Locator {
    return this.documentCards.filter({
      has: this.page.locator(`text=/.${type}$/i`),
    });
  }

  getDocumentsByCategory(category: string): Locator {
    return this.documentCards.filter({
      has: this.page.locator(`text=/${category}/i`),
    });
  }
  ...
  getFavoriteDocuments(): Locator {
    return this.documentCards.filter({
      has: this.page.locator(
        '[class*="fill-"][class*="yellow"], [class*="fill-"][class*="warning"]',
      ),
    });
  }
  ...
  getRecentDocuments(): Locator {
    return this.page
      .locator("text=/Recent|Recently/i")
      .locator("..")
      .locator("div.rounded-lg");
  }
}
