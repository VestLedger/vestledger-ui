...
export class CompliancePage {
  ...
  constructor(page: Page) {
    ...
    this.pageTitle = page.getByRole("heading", {
      level: 1,
      name: /compliance\s*&\s*regulatory/i,
    });
    ...
    // Compliance items
    this.complianceItemsList = page
      .locator("div.rounded-lg")
      .filter({ hasText: /overdue|in-progress|upcoming|completed/i });
    this.complianceItemCards = page
      .locator("div.rounded-lg")
      .filter({ hasText: /compliance|filing|audit/i });

    // Filters
    this.statusFilter = page
      .getByRole("combobox", { name: /status/i })
      .or(page.locator("select").filter({ hasText: /status/i }));
  ...
  async filterByStatus(
    status: "all" | "overdue" | "in-progress" | "upcoming" | "completed",
  ) {
    await this.statusFilter.click();
    await this.page
      .getByRole("option", { name: new RegExp(status.replace("-", " "), "i") })
      .click();
  ...
  async getComplianceItemsByStatus(
    status: "overdue" | "in-progress" | "upcoming" | "completed",
  ) {
    return this.page
      .locator('[class*="badge"], [class*="status"]')
      .filter({ hasText: new RegExp(status.replace("-", " "), "i") });
  }
  ...
  async markItemAsComplete(itemName: string) {
    ...
    const completeButton = itemCard.getByRole("button", {
  ...
  async viewFilingSchedule() {
    ...
    const schedule = this.page.locator(
      "text=/filing.*schedule|regulatory.*calendar/i",
    );
  ...
  async viewAuditSchedule() {
    ...
    const schedule = this.page.locator(
      "text=/audit.*schedule|audit.*calendar/i",
    );
    return schedule.first();
  }
