...
export class Valuation409APage {
  ...
  constructor(page: Page) {
    ...
    this.pageTitle = page.getByRole("heading", {
      level: 1,
      name: /409a valuations?/i,
    });
    this.requestNewValuationButton = page.getByRole("button", {
      name: /request new valuation/i,
    });
    ...
    // Valuations list
    this.valuationCards = page
      .locator("div.rounded-lg")
      .filter({ hasText: /fair market value|FMV|valuation date/i });
  }
  ...
  async downloadValuationReport(companyName: string) {
    const card = await this.getValuationByCompany(companyName);
    const downloadButton = card.getByRole("button", {
  ...
  async getExpiringValuations() {
    return this.page
      .locator("div.rounded-lg")
      .filter({ hasText: /expiring|expires/i });
  }
