...
export class TaxCenterPage {
  ...
  constructor(page: Page) {
    ...
    this.generateK1sButton = page.getByRole("button", {
      name: /generate k-1s/i,
    });
    this.uploadDocumentsButton = page.getByRole("button", {
    ...
    // Tax documents
    this.taxDocumentsList = page
      .locator("div.rounded-lg")
      .filter({ hasText: /K-1|1099|tax/i });
    this.documentCards = page
      .locator("div.rounded-lg")
      .filter({ hasText: /ready|sent|pending/i });

    // Filters
    this.searchInput = page.getByPlaceholder(/search/i);
    this.yearFilter = page
      .getByRole("combobox", { name: /year/i })
      .or(page.locator("select").filter({ hasText: /2024|2023/i }));
  ...
  async getFilingDeadline() {
    return this.filingDeadlineMetric
  ...
  async getDeadlineDaysRemaining() {
    const subtitle = await this.filingDeadlineMetric
      .locator("text=/\\d+\\s+days\\s+remaining/i")
      .first()
      .textContent();
  ...
  async downloadDocument(documentName: string) {
    ...
    const downloadButton = documentCard.getByRole("button", {
      name: /download/i,
    });
