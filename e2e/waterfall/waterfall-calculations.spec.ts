...
test.describe("Waterfall Modeling - Scenarios", () => {
  ...
  test("should display scenario manager panel", async ({ page }) => {
    ...
    const scenarioManager = page
      .locator('[class*="scenario"], [data-testid*="scenario"]')
      .filter({ hasText: /scenario|manage/i });
  ...
  test("should have create scenario button", async ({ page }) => {
    ...
    const createButton = page.getByRole("button", {
      name: /create|new|add.*scenario/i,
    });
  ...
  test("should support favorite/tag functionality", async ({ page }) => {
    ...
    const favoriteButton = page
      .getByRole("button", { name: /favorite|star|bookmark/i })
...
test.describe("Waterfall Modeling - Model Selection", () => {
  test("should display model type selector (European/American/Blended)", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
    ...
    const modelSelector = page
      ...
      .or(
        page
          .locator("select")
          .filter({ hasText: /european|american|blended/i }),
      )
  ...
  test("should show model type badge for selected scenario", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
    ...
    const modelBadge = page
      .locator('[class*="badge"]')
      .filter({ hasText: /EU|US|european|american|blended/i });
...
test.describe("Waterfall Modeling - Tier Management", () => {
  test("should display tier breakdown table", async ({ page }) => {
    ...
    const tierTable = page
      .locator('table, [role="grid"]')
      .filter({ hasText: /tier|ROC|preferred|carry|catch/i });
  ...
  test("should have add tier functionality", async ({ page }) => {
    ...
    const addTierButton = page.getByRole("button", {
      name: /add tier|new tier/i,
    });
    if (await addTierButton.isVisible()) {
      ...
      const tierForm = page
        ...
        .filter({ hasText: /tier|type|percentage/i });
  ...
  test("should allow editing existing tiers", async ({ page }) => {
    ...
    const tierRows = page
      .locator('tr, [class*="tier-row"]')
      .filter({ hasText: /ROC|preferred|carry|catch/i });

    if (await tierRows.first().isVisible()) {
      const editButton = tierRows
        .first()
  ...
  test("should display tier timeline visualization", async ({ page }) => {
    ...
    const timeline = page.locator(
      '[class*="timeline"], [data-testid="tier-timeline"]',
    );
...
test.describe("Waterfall Modeling - Investor Classes", () => {
  ...
  test("should validate ownership percentages sum to 100%", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
  ...
  test("should allow adding new investor classes", async ({ page }) => {
    ...
    const addClassButton = page.getByRole("button", {
      name: /add.*class|add.*investor/i,
    });
...
test.describe("Waterfall Modeling - Calculations", () => {
  test("should have exit value input field", async ({ page }) => {
    ...
    const exitValueInput = page
      .getByLabel(/exit value/i)
  ...
  test("should have calculate/run button", async ({ page }) => {
    ...
    const calculateButton = page.getByRole("button", {
      name: /calculate|run|compute|model/i,
    });
  ...
  test("should display calculation results", async ({ page }) => {
    ...
    const resultsSection = page.locator(
      "text=/result|GP carry|LP return|total return|\$[\\d,]+/i",
    );
...
test.describe("Waterfall Modeling - Sensitivity Analysis", () => {
  ...
  test("should show multiple exit value scenarios", async ({ page }) => {
    ...
    const sensitivityResults = page
      .locator('[class*="sensitivity"], table')
      .filter({ hasText: /\$[\\d,]+.*\$[\\d,]+/i });
...
test.describe("Waterfall Modeling - Charts", () => {
  ...
  test("should have chart type selector", async ({ page }) => {
    ...
    const chartSelector = page
      .locator('[class*="chart-selector"], [role="tablist"]')
      .filter({ hasText: /waterfall|scenario|LP/i });
  ...
  test("should switch between chart types", async ({ page }) => {
    ...
    const chartTabs = page
      .getByRole("tab")
      .or(page.locator('[class*="chart-option"]'));
  ...
  test("should display LP detail chart when selected", async ({ page }) => {
    ...
    const lpDetailTab = page
      .getByRole("tab", { name: /LP detail/i })
...
test.describe("Waterfall Modeling - Scenario Comparison", () => {
  ...
  test("should display scenario comparison view", async ({ page }) => {
    ...
    const comparisonTab = page
      .getByRole("tab", { name: /comparison|scenario/i })
      .or(
        page
          .locator('[class*="chart-option"]')
    ...
    if (await comparisonTab.isVisible()) {
      ...
      const comparisonChart = page.locator(
        '[class*="stacked"], [class*="comparison"]',
      );
...
test.describe("Waterfall Modeling - Interactions - Data Verification", () => {
  test("scenario selector should update calculation results", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
    ...
    const dataSelector =
      '[class*="result"], [data-testid="calculation-result"], table tbody tr';
    ...
    if (await scenarioSelector.first().isVisible()) {
      ...
      if (await scenarioOption.isVisible()) {
        ...
        if (before.count > 0) {
          expect(
            changed,
            "Scenario selection should update calculation results",
          ).toBe(true);
        }
      }
    }
  ...
  test("model type selector should update tier calculations", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
    ...
    const dataSelector =
      'table tbody tr, [class*="tier"], [data-testid="tier-row"]';
    ...
    if (await modelSelector.first().isVisible()) {
      ...
      const modelOption = page.getByRole("option", {
        name: /american|european|blended/i,
      });
      if (await modelOption.first().isVisible()) {
        ...
        if (before.count > 0) {
          expect(
            changed,
            "Model type change should update tier calculations",
          ).toBe(true);
        }
      }
    }
  ...
  test("exit value input should update calculation results", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");
    ...
    const dataSelector =
      '[class*="result"], text=/\$[\\d,]+/, [data-testid="gp-carry"], [data-testid="lp-return"]';
    ...
    if (await exitValueInput.isVisible()) {
      ...
      if (before.count > 0) {
        expect(
          changed,
          "Exit value change should update calculation results",
        ).toBe(true);
      }
    }
  });
  ...
  test("chart type tabs should update displayed chart", async ({ page }) => {
    ...
    if ((await chartTabs.count()) > 1) {
      ...
      expect(changed, "Chart tab switch should update displayed chart").toBe(
        true,
      );
    }
  });

  test("sensitivity analysis should update with different parameters", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");

    const sensitivityInputs = page.locator(
      '[class*="sensitivity"] input, [data-testid="sensitivity-input"]',
    );
    const dataSelector =
      '[class*="sensitivity"] table, [class*="sensitivity-results"]';
    ...
    if (await sensitivityInputs.first().isVisible()) {
      ...
      if (before.count > 0) {
        expect(
          changed,
          "Sensitivity parameter change should update results",
        ).toBe(true);
      }
    }
  });

  test("investor class changes should recalculate distributions", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/waterfall");
    await page.waitForLoadState("networkidle");

    const investorInput = page
      .locator("input")
      .filter({ hasText: /LP|GP|ownership/i })
      .first()
      .or(page.locator('[data-testid="investor-percentage"]'));

    const dataSelector =
      '[class*="result"], [data-testid="distribution-result"]';
    ...
    if (await investorInput.isVisible()) {
      ...
      const calculateButton = page.getByRole("button", {
        name: /calculate|run|recalculate/i,
      });
      if (await calculateButton.isVisible()) {
        await calculateButton.click();
        await page.waitForLoadState("networkidle");
      ...
      if (before.count > 0) {
        expect(
          changed,
          "Investor class change should recalculate distributions",
        ).toBe(true);
      }
    }
  });
