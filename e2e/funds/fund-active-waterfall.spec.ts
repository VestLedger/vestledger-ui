...
test.describe("Fund Active Waterfall", () => {
  test("should display Active Waterfall row in fund details", async ({
    page,
  }) => {
    ...
    if (await fundRow.isVisible()) {
      ...
      await expect(fundAdmin.activeWaterfallLabel).toBeVisible({
        timeout: 10000,
      });

      // Should show either a scenario name or "None set"
      const waterfallValue = page
        .getByText("None set")
  ...
  test("should show Change button when fund is selected", async ({ page }) => {
    ...
    if (await fundRow.isVisible()) {
      ...
      if (await fundAdmin.activeWaterfallLabel.isVisible()) {
        ...
        await expect(fundAdmin.changeWaterfallButton).toBeVisible({
          timeout: 5000,
        });
      }
    }
  ...
  test("should display fund performance alongside active waterfall", async ({
    page,
  }) => {
