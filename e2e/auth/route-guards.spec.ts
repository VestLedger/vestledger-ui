...
test.describe('Route Guards - Unauthenticated Redirect', () => {
  for (const route of DASHBOARD_ROUTES) {
    test(`unauthenticated visit to ${route} should redirect to /login`, async ({ page }) => {
...
test.describe('Route Guards - Authenticated Access (gp)', () => {
  test('authenticated gp user can visit all accessible routes', async ({ page, baseURL }) => {
    ...
    for (const route of GP_ACCESSIBLE_ROUTES) {
      ...
      expect(currentPath, `Expected to land on ${route}, got ${currentPath}`).not.toBe('/login');
      expect(
...
test.describe('Route Guards - Role-Denied Routes (gp)', () => {
  test('gp user is redirected away from restricted routes', async ({ page, baseURL }) => {
    const appOrigin = new URL(baseURL || 'http://localhost:3000').origin;
    const storageState = await createApiStorageState(appOrigin, getTestUser());
    ...
    for (const route of GP_DENIED_ROUTES) {
      ...
      const currentPath = new URL(page.url()).pathname;
      expect(currentPath, `Should not stay on denied route ${route}`).not.toBe(route);
      expect(currentPath, `Should not be on /login (user is authenticated)`).not.toBe('/login');
    }
  });
});
