...
test.describe("Deal Intelligence - AI Deal Sourcing", () => {
  ...
  test("should have company search functionality", async ({ page }) => {
    ...
    const searchSection = page
      .locator('[class*="search"], [class*="Search"]')
...
test.describe("Deal Intelligence - Active Deals", () => {
  test("should display active deals section", async ({ page }) => {
    ...
    const activeDealsSection = page.locator(
      "text=/Active Deals.*Due Diligence/i",
    );
...
test.describe("Deal Intelligence - Per-Deal View", () => {
  test("should navigate to deal detail view on click", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      await expect(dealIntel.backToFundViewButton).toBeVisible({
        timeout: 10000,
      });
    }
  });
...
test.describe("Deal Intelligence - Per-Deal Tabs", () => {
  ...
  test("should switch to analytics tab", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const analyticsContent = page.locator(
        "text=/Financial Metrics|Market Analytics/i",
      );
  ...
  test("should switch to documents tab", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const documentsContent = page.locator(
        "text=/Document Library|Search documents/i",
      );
  ...
  test("should show category completion status", async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const statusBadge = page
        .locator('[class*="badge"]')
        .filter({ hasText: /completed|pending|overdue/i });
...
test.describe("Deal Intelligence - IC Status", () => {
  test("should display IC status badges on deals", async ({ page }) => {
    ...
    const statusBadge = page
      .locator('[class*="badge"]')
      .filter({ hasText: /ready.*ic|dd.*progress/i });
    if ((await statusBadge.count()) > 0) {
      await expect(statusBadge.first()).toBeVisible();
    }
  });
...
test.describe("Deal Intelligence - Interactions - Data Verification", () => {
  test("clicking deal card should update view to show deal details", async ({
    page,
  }) => {
    const dealIntel = new DealIntelligencePage(page);
    await dealIntel.goto();
    ...
    const dataSelector =
      'div.rounded-lg, [class*="content"], [class*="panel"], h2, h3';
  ...
  test("tab navigation in deal view should update content", async ({
    page,
  }) => {
    const dealIntel = new DealIntelligencePage(page);
    await dealIntel.goto();
    ...
    if (count > 0) {
      ...
      const dataSelector =
        'div.rounded-lg, table, [class*="content"], [class*="panel"]';
      ...
      const overviewToAnalytics = verifyDataChanged(
        overviewSnapshot,
        analyticsSnapshot,
      );
      const analyticsToDocuments = verifyDataChanged(
        analyticsSnapshot,
        documentsSnapshot,
      );
  ...
  test("stage filter should update deal list", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="deal-card"], table tbody tr';
    ...
    if (await stageFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await option.isVisible()) {
          ...
          expect(changed, "Stage filter should update deal list").toBe(true);
        }
      }
    }
  });
  ...
  test("sector filter should update deal list", async ({ page }) => {
    ...
    const sectorFilter = page
      .getByRole("combobox", { name: /sector/i })
    ...
    if (await sectorFilter.first().isVisible()) {
      ...
      if (before.count > 0) {
        ...
        if (await option.isVisible()) {
          ...
          expect(changed, "Sector filter should update deal list").toBe(true);
        }
      }
    }
  });
  ...
  test("AI company search should show results", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg, [class*="result"], [class*="suggestion"]';
    ...
    if (await searchInput.first().isVisible()) {
      ...
      expect(
        changed || after.count > 0,
        "Search should produce results or suggestions",
      ).toBe(true);
    }
  });

  test("document search in deal view should filter documents", async ({
    page,
  }) => {
    const dealIntel = new DealIntelligencePage(page);
    await dealIntel.goto();
    ...
    if (count > 0) {
      ...
      const dataSelector =
        '[class*="document"], [class*="file"], table tbody tr';
  ...
  test("back to fund view should restore original content", async ({
    page,
  }) => {
    const dealIntel = new DealIntelligencePage(page);
    await dealIntel.goto();
    ...
    if (count > 0) {
      ...
      const backToOriginal =
        !verifyDataChanged(fundViewSnapshot, afterBackSnapshot) ||
