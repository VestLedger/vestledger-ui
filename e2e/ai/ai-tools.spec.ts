...
test.describe("AI Tools - Page Load", () => {
  test("should load AI tools page", async ({ page }) => {
    ...
    const pageTitle = page
      ...
      .filter({ hasText: /ai|tools|assistant/i });
  ...
  test("should display AI tools content", async ({ page }) => {
    ...
    await expect(getContextualTab(page, /ai decision writer/i)).toBeVisible({
      timeout: 10000,
    });
...
test.describe("AI Tools - Available Tools", () => {
  test("should display available AI tool cards", async ({ page }) => {
    ...
    const toolCards = page.locator(
      '[data-testid="ai-tool"], div.rounded-lg, [class*="tool"]',
    );
  ...
  test("should show tool names and descriptions", async ({ page }) => {
    ...
    const toolNames = page.locator(
      "text=/document.*analysis|pitch.*deck|due.*diligence|copilot|assistant|insights/i",
    );
...
test.describe("AI Tools - Document Analysis", () => {
  test("should have document analysis tool", async ({ page }) => {
    ...
    const docAnalysis = page.locator(
      "text=/document.*analysis|analyze.*document/i",
    );
...
test.describe("AI Tools - Pitch Deck Reader", () => {
  test("should have pitch deck analysis tool", async ({ page }) => {
    ...
    const pitchDeck = page.locator(
      "text=/pitch.*deck|deck.*reader|presentation/i",
    );
...
test.describe("AI Tools - Insights", () => {
  ...
  test("should show recent AI-generated insights", async ({ page }) => {
    ...
    const recentInsights = page.locator(
      '[data-testid="ai-insight"], [class*="insight"]',
    );
...
test.describe("AI Tools - Task Prioritizer", () => {
  test("should have AI task prioritization", async ({ page }) => {
    ...
    const taskPrioritizer = page.locator(
      "text=/task.*priorit|smart.*task|ai.*task/i",
    );
...
test.describe("AI Tools - Decision Writer", () => {
  test("should have decision writing tool", async ({ page }) => {
    ...
    const decisionWriter = page.locator(
      "text=/decision.*writer|investment.*memo|write.*decision/i",
    );
...
test.describe("AI Tools - Settings", () => {
  test("should have AI settings/preferences", async ({ page }) => {
    ...
    const settingsButton = page.getByRole("button", {
      name: /settings|preferences|configure/i,
    });
...
test.describe("AI Tools - History", () => {
  test("should show AI interaction history", async ({ page }) => {
    ...
    const historySection = page.locator(
      "text=/history|recent|past.*interactions/i",
    );
...
test.describe("AI Tools - Interactions - Data Verification", () => {
  test("tool category filter should update displayed tools", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/ai-tools");
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="ai-tool"], [class*="tool"]';
  ...
  test("clicking AI tool card should show tool details or launch", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/ai-tools");
    ...
    if ((await toolCards.count()) > 0) {
      const detailsSelector =
        '[role="dialog"], [class*="drawer"], [class*="detail"], [class*="panel"], [class*="modal"], [class*="chat"]';
  ...
  test("tab navigation should update AI tools view", async ({ page }) => {
    ...
    if (
      (await decisionWriterTab.count()) > 0 &&
      (await pitchDeckTab.count()) > 0
    ) {
      const before = await captureDataSnapshot(page, dataSelector);
  ...
  test("clicking upload button should open upload interface", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/ai-tools");
    ...
    if (await uploadButton.first().isVisible()) {
      const dataSelector =
        '[role="dialog"], [class*="modal"], [class*="upload"], input[type="file"]';
      const before = await captureDataSnapshot(page, dataSelector);
      ...
      expect(changed, "Upload button should open upload interface").toBe(true);
    }
  });
  ...
  test("settings button should open settings panel", async ({ page }) => {
    ...
    if (await settingsButton.first().isVisible()) {
      const dataSelector =
        '[role="dialog"], [class*="modal"], [class*="settings"], [class*="panel"], [class*="drawer"]';
      const before = await captureDataSnapshot(page, dataSelector);
      ...
      expect(changed, "Settings button should open settings panel").toBe(true);
    }
  });
});
