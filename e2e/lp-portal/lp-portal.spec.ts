...
test.describe("LP Portal", () => {
  ...
  test("should display LP portal content", async ({ page }) => {
    ...
    const content = page.locator(
      '[data-testid="lp-portal"], [class*="portal"], main',
    );
  ...
  test("should show investments summary", async ({ page }) => {
    ...
    const investments = page.locator(
      '[data-testid="investments"], [class*="investment"], table',
    );
  ...
  test("should display portfolio value", async ({ page }) => {
    ...
    const valueDisplay = page.locator(
      '[data-testid="portfolio-value"], [class*="value"], [class*="total"]',
    );
...
test.describe("LP Portal Interactions", () => {
  test("should be able to view investment details", async ({ page }) => {
    ...
    const investmentItem = page
      .locator(
        '[data-testid="investment-item"], table tbody tr, [class*="investment-card"]',
      )
      .first();
...
test.describe("LP Portal - Interactions - Data Verification", () => {
  test("fund selector should update portfolio data", async ({ page }) => {
    ...
    const dataSelector =
      'div.rounded-lg, [data-testid="investment"], table tbody tr';
    ...
    if (await fundSelector.first().isVisible()) {
      ...
      if (await fundOption.isVisible()) {
        ...
        if (before.count > 0) {
          expect(changed, "Fund selector should update portfolio data").toBe(
            true,
          );
        }
      }
  ...
  test("tab navigation should update displayed content", async ({ page }) => {
    ...
    expect(changed, "Tab navigation should update displayed content").toBe(
      true,
    );
  });

  test("period selector should update performance metrics", async ({
    page,
  }) => {
    await loginViaRedirect(page, "/lp-portal");
    await page.waitForLoadState("networkidle");
    ...
    const dataSelector =
      '[class*="metric"], [class*="value"], [data-testid="performance"]';
    ...
    if (await periodSelector.first().isVisible()) {
      ...
      if (await periodOption.isVisible()) {
        ...
        if (before.count > 0) {
          expect(
            changed,
            "Period selector should update performance metrics",
          ).toBe(true);
        }
      }
    }
  ...
  test("clicking investment should show details panel", async ({ page }) => {
    ...
    const investmentItems = page.locator(
      '[data-testid="investment-item"], table tbody tr, [class*="investment-card"]',
    );

    if ((await investmentItems.count()) > 0) {
      const detailsSelector =
        '[role="dialog"], [class*="drawer"], [class*="detail"], [class*="panel"]';
      ...
      expect(changed, "Clicking investment should show details panel").toBe(
        true,
      );
    }
  });
