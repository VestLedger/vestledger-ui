...
test.describe("Portfolio", () => {
  ...
  test("should display portfolio content", async ({ page }) => {
    ...
    const content = page.locator(
      'table, [data-testid="portfolio"], [class*="portfolio"]',
    );
  ...
  test("should display summary cards", async ({ page }) => {
    ...
    const summaryCards = page.locator(
      '[data-testid="summary-card"], [class*="stat"], [class*="summary"]',
    );
  ...
  test("should be able to view company details", async ({ page }) => {
    ...
    const companyItem = page
      .locator('table tbody tr, [data-testid="company-item"]')
      .first();
...
test.describe("Portfolio Interactions - Data Verification", () => {
  test("fund selector should update displayed data", async ({ page }) => {
    ...
    const dataSelector =
      'table tbody tr, [data-testid="portfolio-item"], [data-testid="company-card"]';
    ...
    if (await filterButton.isVisible()) {
      ...
      if (await filterOption.isVisible()) {
        ...
        if (before.count > 0) {
          ...
          expect(
            changed,
            "Filter selection should update displayed items",
          ).toBe(true);
        }
      }
    }
