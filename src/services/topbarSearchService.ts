...
export async function searchTopbar(
  query: string,
): Promise<TopbarSearchResult[]> {
  ...
  for (const document of documents) {
    mappedResults.push({
      ...
      title: document.name,
      description:
        [document.category, formatUploadedDate(document.uploadedDate)]
          .filter(Boolean)
          .join(" • ") || "Document",
      category: "Documents",
      icon: FileText,
  ...
  for (const fund of funds) {
    mappedResults.push({
      ...
      description:
        [fund.strategy, fund.status].filter(Boolean).join(" • ") || "Fund",
      category: "Funds",
      icon: Building2,
      href: "/analytics",
    });
