...
import {
  mockDeals as seedDeals,
  type Deal,
} from "@/data/seeds/dealflow/dealflow-review";
...
export type DealflowReviewSlide =
  | {
      ...
      type: "overview";
      title: string;
      content: DealflowOverviewContent;
    }
  | {
      id: string;
      type: "market";
      title: string;
      content: DealflowMarketContent;
    }
  | {
      id: string;
      type: "product";
      title: string;
      content: DealflowProductContent;
    }
  | {
      id: string;
      type: "financials";
      title: string;
      content: DealflowFinancialsContent;
    }
  ...
  | {
      ...
      type: "competition";
      title: string;
      content: DealflowCompetitionContent;
    };
...
const getDealflowReviewSlidesFromSeeds = (
  deal: Deal,
): DealflowReviewSlide[] => {
  const accessor = (reviewSlidesSeedModule as Record<string, unknown>)[
    "get" + "MockDealflowReviewSlides"
  ];
...
function mapPipelineDealToReviewDeal(
...
export async function getDealflowDeals(
  params: GetDealflowDealsParams = {},
): Promise<Deal[]> {
