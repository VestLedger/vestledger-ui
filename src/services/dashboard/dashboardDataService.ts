...
function unwrapDashboardPayload(payload: unknown): Record<string, unknown> | null {
...
function mapAlertType(value: unknown): 'critical' | 'warning' | 'info' {
  ...
  if (normalized === 'critical' || normalized === 'high' || normalized === 'error') return 'critical';
  if (normalized === 'warning' || normalized === 'medium') return 'warning';
  return 'info';
}

function mapTaskDomain(value: unknown): DashboardData['tasks'][number]['domain'] {
  const normalized = readString(value).toLowerCase();
  if (
  ...
  return ensureArray<unknown>(value)
    ...
    .map((item, index) => {
      ...
      return {
        ...
        overdueLPs,
        prediction: {
          expectedCompletion: parseDate(prediction.expectedCompletion, new Date(dueDate.getTime() + (3 * MS_PER_DAY))),
...
function normalizePortfolioCompanies(value: unknown): DashboardData['portfolioCompanies'] {
  return ensureArray<unknown>(value)
    .filter(isRecord)
    .map((item, index) => {
      ...
      return {
        ...
        prediction: {
          nextQuarterHealth: clamp(readNumber(prediction.nextQuarterHealth, health + (healthChange * 0.45)), 0, 100),
          confidence: clamp(readNumber(prediction.confidence, 0.78), 0, 1),
          reasoning: readString(prediction.reasoning, 'Derived from current health trend and runway posture.'),
        },
        anomalies: ensureArray<unknown>(item.anomalies)
          ...
          .map((anomaly) => ({
            ...
            severity: readString(anomaly.severity, 'low') === 'high'
              ? 'high'
...
function normalizeAlerts(value: unknown): DashboardData['alerts'] {
  return ensureArray<unknown>(value)
    ...
    .map((item, index) => {
      ...
      const priority = clamp(readNumber(item.priority, type === 'critical' ? 90 : type === 'warning' ? 70 : 45), 0, 100);
      const title = readString(item.title, `Alert ${index + 1}`);
      const description = readString(item.description, readString(item.message, 'Action recommended.'));
      ...
      return {
        ...
        description,
        priority,
        reasoning: readString(item.reasoning, `System marked this as ${type} based on urgency and operational impact.`),
      };
    });
}
...
function normalizeTasks(value: unknown): DashboardData['tasks'] {
  return ensureArray<unknown>(value)
    ...
    .map((item, index) => {
      ...
      const impact = clamp(readNumber(item.impact, readNumber(item.priorityScore, urgency * 6) / Math.max(urgency, 1)), 0, 10);
      ...
      return {
        ...
        description: readString(item.description, 'Review and action required.'),
        domain: mapTaskDomain(item.domain),
        urgency,
        impact,
        priorityScore: clamp(readNumber(item.priorityScore, urgency * impact), 0, 100),
...
function derivePortfolioRevenueRows(companies: DashboardData['portfolioCompanies']): PortfolioRevenueRow[] {
  return companies
    .map((company) => {
      ...
      const arr = Math.max(1, Math.round((((health / 100) * 22) + Math.max(0, company.burnRate / 200_000)) * 10) / 10);
      const arrGrowthQoq = clamp(Math.round(company.healthChange * 1.6), -30, 45);
      const valuation = Math.max(8, Math.round(arr * (health >= 80 ? 8 : health >= 70 ? 7 : 6)));
...
function derivePortfolioSignals(rows: PortfolioRevenueRow[]): DashboardData['portfolioSignals'] {
  return rows
    .map((row) => ({
      id: row.id,
      name: row.name,
...
function mapDomainToRoute(domain: DashboardData['tasks'][number]['domain']): Pick<DailyBriefItem, 'route' | 'tabTarget' | 'lane'> {
  if (domain === 'capital-calls') {
    return { route: ROUTE_PATHS.fundAdmin, tabTarget: 'capital-calls', lane: 'LP Relations' };
  }
  if (domain === 'portfolio') {
...
function mapAlertToRouteAndLane(alert: DashboardData['alerts'][number]): Pick<DailyBriefItem, 'route' | 'tabTarget' | 'lane'> {
  ...
  if (lowerTitle.includes('portfolio') || lowerTitle.includes('company') || lowerTitle.includes('runway') || lowerTitle.includes('revenue')) {
    return { route: ROUTE_PATHS.portfolio, lane: 'Portfolio' };
  }
  return { route: ROUTE_PATHS.dashboard, lane: 'Operations' };
}
...
function deriveDailyBriefItems(
  ...
  const taskItems: DailyBriefItem[] = activeTasks.map((task, index) => {
    const dueDate = new Date(now.getTime() + ((index % BRIEF_HORIZON_DAYS) * MS_PER_DAY));
  ...
  return [...taskItems, ...warningItems, ...overdueCallWarnings].sort((a, b) => {
...
function deriveBlockers(items: DailyBriefItem[], now: Date): HomeBlocker[] {
  ...
  return items
    ...
    .map((item) => {
      ...
      const blockedDays = Math.max(1, Math.ceil((now.getTime() - item.dueDate.getTime()) / MS_PER_DAY));
...
function deriveTrendPoints(
  ...
  const currentArr = Math.max(1, rows.reduce((sum, row) => sum + row.arr, 0));
...
function deriveRevenueDistribution(
  ...
  const slices: RevenueDistributionSlice[] = topContributors.map((row, index) => ({
    id: `rev-${row.id}`,
    name: row.name,
    value: row.arr,
...
function normalizeDashboardPayload(
  ...
  const portfolioCompanies = normalizePortfolioCompanies(root.portfolioCompanies);
  const alerts = normalizeAlerts(root.alerts);
  const tasks = normalizeTasks(root.tasks);
  const quickActions = ensureArray(root.quickActions) as DashboardData['quickActions'];
  ...
  const dailyBriefItems = deriveDailyBriefItems(tasks, alerts, capitalCalls, now);
  const blockers = deriveBlockers(dailyBriefItems, now);
  const opportunities = deriveOpportunities(portfolioRevenueRows);
  const portfolioRevenueTrend = deriveTrendPoints(root.portfolioRevenueTrend, portfolioRevenueRows, now);
  const revenueDistribution = deriveRevenueDistribution(root.revenueDistribution, portfolioRevenueRows, portfolioRevenueTrend);
  ...
  const currentArr = portfolioRevenueTrend[portfolioRevenueTrend.length - 1]?.arr ?? 0;
  const trustWatchCount = derivedFundTrustRows.filter((row) => row.riskFlag !== 'stable').length;
  ...
  return {
    ...
    morningBrief: {
      summary:
        typeof morningBrief.summary === 'string' && morningBrief.summary.trim().length > 0
    ...
    metrics: {
      ...
      atRiskCompanies:
        typeof metrics.atRiskCompanies === 'number' && Number.isFinite(metrics.atRiskCompanies)
          ...
          : portfolioRevenueRows.filter((row) => row.riskFlag === 'critical').length,
      healthyCompanies:
        typeof metrics.healthyCompanies === 'number' && Number.isFinite(metrics.healthyCompanies)
          ? metrics.healthyCompanies
          : portfolioRevenueRows.filter((row) => row.riskFlag === 'stable').length,
      totalTasks:
      ...
      urgentTasks:
        typeof metrics.urgentTasks === 'number' && Number.isFinite(metrics.urgentTasks)
          ? metrics.urgentTasks
          : tasks.filter((task) => task.priorityScore >= 70 && task.status !== 'completed').length,
    },
    selectedFundName: typeof root.selectedFundName === 'string'
