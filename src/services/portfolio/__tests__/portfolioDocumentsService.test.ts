...
describe("portfolioDocumentsService", () => {
  ...
  it("returns mock snapshot in mock mode", async () => {
    const service =
      await import("@/services/portfolio/portfolioDocumentsService");
  ...
  it("maps API documents and applies company status counts in api mode", async () => {
    ...
    fetchPortfolioSnapshot.mockResolvedValue({
      companies: [
        {
          id: "co-1",
          companyName: "Axiom Health",
          sector: "HealthTech",
          stage: "Series A",
        },
        {
          id: "co-2",
          companyName: "BridgeCore",
          sector: "FinTech",
          stage: "Seed",
        },
      ],
    ...
    expect(snapshot.companies.some((company) => company.pendingCount > 0)).toBe(
      true,
    );
  });
