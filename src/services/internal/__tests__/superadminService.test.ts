...
describe("internal superadmin services", () => {
  ...
  it("onboards a tenant and creates first-admin invitation", () => {
    ...
    expect(
      tenants.some((tenant) => tenant.displayName === "Apex Ventures"),
    ).toBe(true);
  });

  it("updates tenant lifecycle status", () => {
    const targetTenant = listTenants().find(
      (tenant) => tenant.id === "org_summit_vc",
    );
  ...
  it("enforces last-admin guard for role demotion and disable", () => {
    ...
    const onlyAdmin = snapshot.members.find(
      (member) => member.organizationRole === "org_admin",
    );
