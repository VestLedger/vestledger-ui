...
const formatAmountToMillions = vi.fn(
  (amount: number) => `$${amount / 1_000_000}M`,
);
...
describe("pipelineService", () => {
  ...
  it("loads API-backed pipeline data in api mode", async () => {
    ...
    expect(fetchPipelineDealsFromApi).toHaveBeenCalledWith({
      stageFilter: "Sourced",
    });
  ...
  it("updates mock deal stage and throws when deal id is unknown", async () => {
    const service = await import("@/services/pipelineService");
    const updated = await service.updatePipelineDealStage(
      pipelineDeals[0].id,
      "Closed",
    );
    ...
    await expect(
      service.updatePipelineDealStage("missing-id", "Closed"),
    ).rejects.toThrow("Pipeline deal not found: missing-id");
  });
  ...
  it("creates and updates via API gateway in api mode", async () => {
    ...
    expect(updatePipelineDealInApi).toHaveBeenCalledWith("api-created", {
      stage: "Due Diligence",
    });
