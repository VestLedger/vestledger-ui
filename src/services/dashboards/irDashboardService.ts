...
export async function getIRDashboardSnapshot() {
  if (isMockMode("dashboards")) {
    return {
      metrics: irDashboardMetrics,
      recentInteractions: irRecentInteractions,
      upcomingTasks: irUpcomingTasks,
    };
  }

  // API mode
  const result = await unwrapApiResult(apiClient.GET("/dashboard/ir"), {
  ...
  const lpEngagement = isRecord(apiData.lpEngagement)
    ? apiData.lpEngagement
    : {};
  ...
  const totalLPs = readNumber(
    metricsSource.totalLPs,
    readNumber(lpEngagement.totalLps),
  );
  const activeLPs = readNumber(
    metricsSource.activeLPs,
    readNumber(lpEngagement.activeLps),
  );
  const engagementScore = readNumber(
    metricsSource.engagementScore,
    readNumber(lpEngagement.averageEngagementScore),
  );
  const pendingRequests = readNumber(
    metricsSource.pendingRequests,
    pendingCapitalCalls.length || upcomingTasksApi.length,
  );
  ...
  if (metrics[2]) metrics[2].value = String(recentInteractionsApi.length);
  if (metrics[3])
    metrics[3].value = `${Math.max(0, Math.min(100, Math.round(engagementScore)))}%`;
  if (metrics[1])
    metrics[1].change = pendingRequests > 0 ? "Action Req" : "Clear";

  const normalizedInteractions = recentInteractionsApi
    .map((item) => ({
      lp: readString(
        item.lp,
        readString(item.lpName, readString(item.name, "LP")),
      ),
      type: readString(item.type, "Interaction"),
      date: toDisplayDate(item.date),
      notes: readString(
        item.notes,
        readString(item.description, "Recent interaction logged."),
      ),
    }))
    .filter((item) => item.lp.length > 0);

  const normalizedUpcomingTasks = (
    upcomingTasksApi.length > 0 ? upcomingTasksApi : upcomingEvents
  )
    .map((item) => ({
  ...
  return {
    metrics,
    recentInteractions: normalizedInteractions,
    upcomingTasks: normalizedUpcomingTasks,
  };
}
