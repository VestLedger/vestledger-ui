...
function formatDueLabel(value: unknown): string {
  ...
  if (parsed && !Number.isNaN(parsed.getTime())) {
    ...
    const startOfDue = new Date(parsed.getFullYear(), parsed.getMonth(), parsed.getDate()).getTime();
    const dayDiff = Math.round((startOfDue - startOfToday) / (24 * 60 * 60 * 1000));
...
export async function getAnalystDashboardSnapshot() {
  ...
  const portfolioOverview = isRecord(apiData.portfolioOverview) ? apiData.portfolioOverview : {};
  ...
  const activeDeals = readNumber(metricsSource.activeDealsPipeline, readNumber(portfolioOverview.totalCompanies));
  const dealsUnderDD = readNumber(metricsSource.dealsUnderDD, pendingDueDiligence.length);
  const pendingMemos = readNumber(metricsSource.pendingMemos, urgentTasksApi.length || pendingDueDiligence.length);
  const averageHealthScore = readNumber(metricsSource.averageHealthScore, readNumber(portfolioOverview.averageHealth));
  ...
  if (metrics[2]) metrics[2].value = String(Math.max(recentUpdates.length, recentDealsApi.length));
  ...
  const normalizedRecentDeals = (recentDealsApi.length > 0 ? recentDealsApi : recentUpdates)
    .map((deal) => ({
      name: readString(deal.name, readString(deal.companyName, 'Deal')),
      stage: readString(deal.stage, 'Under Review'),
      score: Math.max(1, Math.min(100, Math.round(readNumber(deal.score, readNumber(deal.health, 82))))),
      sector: readString(deal.sector, 'General'),
    }))
    .filter((deal) => deal.name.trim().length > 0);

  const normalizedUrgentTasks = (urgentTasksApi.length > 0 ? urgentTasksApi : pendingDueDiligence)
    .map((task) => ({
      task: readString(task.task, readString(task.title, `Review ${readString(task.companyName, 'item')}`)).trim(),
  ...
  return {
    metrics,
    recentDeals: normalizedRecentDeals.length > 0 ? normalizedRecentDeals : analystRecentDeals,
    urgentTasks: normalizedUrgentTasks.length > 0 ? normalizedUrgentTasks : analystUrgentTasks,
  };
}
