...
function normalizeTrendSentiment(
  value: unknown,
): "Hot" | "Rising" | "Mixed" | "Stable" {
...
export async function getResearcherDashboardSnapshot() {
  if (isMockMode("dashboards")) {
    return {
      ...
      recentReports: researcherRecentReports,
      trendingTopics: researcherTrendingTopics,
    };
  }

  // API mode
  const result = await unwrapApiResult(apiClient.GET("/dashboard/researcher"), {
  ...
  const sectorsTracked = readNumber(
    metricsSource.sectorsTracked,
    sectorAnalysis.length,
  );
  const trendsIdentified = readNumber(
    metricsSource.trendsIdentified,
    topPerformers.length || trendingTopicsApi.length,
  );
  const reportsGenerated = readNumber(
    metricsSource.reportsGenerated,
    recentReportsApi.length || 24,
  );
  ...
  if (metrics[3])
    metrics[3].value = String(
      Math.max(
        0,
        Math.round((totalCompaniesAnalyzed / Math.max(sectorsTracked, 1)) * 10),
      ),
    );
  ...
  const normalizedRecentReports = recentReportsApi
    .map((report) => ({
      name: readString(
        report.name,
        readString(report.title, "Research report"),
      ),
  ...
  const normalizedTrendingTopics = (
    trendingTopicsApi.length > 0 ? trendingTopicsApi : sectorAnalysis
  )
    .map((topic) => {
      const mentions = readNumber(
        topic.mentions,
        readNumber(topic.companies, 0),
      );
  ...
  return {
    metrics,
    recentReports:
      normalizedRecentReports.length > 0
        ? normalizedRecentReports
        : researcherRecentReports,
    trendingTopics:
      normalizedTrendingTopics.length > 0
        ? normalizedTrendingTopics
        : researcherTrendingTopics,
  };
}
