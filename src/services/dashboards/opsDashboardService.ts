...
export async function getOpsDashboardSnapshot() {
  if (isMockMode("dashboards")) {
    return {
      ...
      complianceAlerts: opsComplianceAlerts,
      upcomingDistributions: opsUpcomingDistributions,
    };
  }

  // API mode
  const result = await unwrapApiResult(apiClient.GET("/dashboard/ops"), {
  ...
  const operationalMetrics = isRecord(apiData.operationalMetrics)
    ? apiData.operationalMetrics
    : {};
  const complianceStatus = isRecord(apiData.complianceStatus)
    ? apiData.complianceStatus
    : {};
  ...
  const activeCapitalCalls = readNumber(
    metricsSource.activeCapitalCalls,
    readNumber(operationalMetrics.activeCapitalCalls),
  );
  const pendingDistributions = readNumber(
    metricsSource.pendingDistributions,
    readNumber(operationalMetrics.pendingDistributions),
  );
  const overdueResponses = readNumber(
    metricsSource.overdueResponses,
    readNumber(operationalMetrics.overdueResponses),
  );
  const complianceRate = readNumber(
    metricsSource.complianceRate,
    readNumber(complianceStatus.totalFunds) > 0
      ? Math.round(
          (readNumber(complianceStatus.compliantFunds) /
            readNumber(complianceStatus.totalFunds)) *
            100,
        )
      : 100,
  );
  ...
  if (metrics[1])
    metrics[1].value = `${Math.max(0, Math.min(100, Math.round(complianceRate)))}%`;
  if (metrics[2]) metrics[2].value = formatCurrency(pendingDistributions);
  if (metrics[3]) metrics[3].value = String(overdueResponses);
  if (metrics[3])
    metrics[3].change = overdueResponses > 0 ? "Action Req" : "Clear";
  ...
  const normalizedComplianceAlerts = complianceAlertsApi
    .map((alert) => ({
      ...
      description: readString(
        alert.description,
        readString(alert.message, "Action required."),
      ),
      severity: normalizeSeverity(alert.severity ?? alert.type),
    }))
    .filter((alert) => alert.title.length > 0);

  const normalizedUpcomingDistributions = (
    upcomingDistributionsApi.length > 0
      ? upcomingDistributionsApi
      : upcomingDeadlines
  )
    .map((dist) => ({
      event: readString(
        dist.event,
        readString(dist.fundName, readString(dist.description, "Distribution")),
      ),
