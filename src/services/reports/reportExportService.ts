...
function normalizeTemplate(raw: unknown, index: number): ReportTemplate {
  ...
  return {
    ...
    name:
      typeof raw.name === "string" ? raw.name : `Report Template ${index + 1}`,
    ...
    estimatedPages:
      typeof raw.estimatedPages === "number" &&
      Number.isFinite(raw.estimatedPages)
        ? raw.estimatedPages
        : undefined,
  };
}
...
export async function createExportJob(
  ...
  if (isMockMode("reports")) {
    return {
      ...
      templateId,
      reportName:
        seedReportTemplates.find((t) => t.id === templateId)?.name ?? "Report",
      format,
      status: "queued",
      progress: 0,
      createdAt: new Date().toISOString(),
