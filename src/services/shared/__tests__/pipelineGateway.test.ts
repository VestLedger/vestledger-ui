...
describe("pipelineGateway", () => {
  ...
  it("formats relative timestamps for common intervals", async () => {
    ...
    expect(gateway.formatRelativeTimestamp("2026-02-14T10:00:00.000Z")).toBe(
      "today",
    );
    expect(gateway.formatRelativeTimestamp("2026-02-13T12:00:00.000Z")).toBe(
      "1 day ago",
    );
    expect(gateway.formatRelativeTimestamp("2026-01-01T00:00:00.000Z")).toBe(
      "1 month ago",
    );
  ...
  it("creates and updates deals through API gateway endpoints", async () => {
    ...
    const updated = await gateway.updatePipelineDealInApi("new-deal", {
      stage: "Closed",
    });
