...
export class UnavailableActionError extends Error {
  ...
  constructor(actionId: UnavailableActionId, context: UnavailableActionContext, message?: string) {
...
export function isUnavailableActionError(error: unknown): error is UnavailableActionError {
...
export function createUnavailableActionError(
  ...
  emitRecoverableErrorToast(toNormalizedError(error), {
    ...
    fallbackMessage: 'This workflow is planned but not wired to a backend endpoint yet.',
