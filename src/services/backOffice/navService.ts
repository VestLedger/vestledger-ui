...
import { mockNAVCalculations } from "@/data/seeds/back-office/fund-admin-ops";
...
export async function getNAVCalculations(
  fundId?: string,
): Promise<NAVCalculation[]> {
  if (isMockMode("backOffice")) {
    const values = fundId
      ? navStore.filter((item) => item.fundId === fundId)
      : navStore;
    return clone(values);
  }
...
export async function calculateNAV(
  ...
  if (isMockMode("backOffice")) {
    ...
    const navPerShare = Number(
      (netAssets / Math.max(outstandingShares, 1)).toFixed(2),
    );
    ...
    const calculation: NAVCalculation = {
      ...
      changeAmount: latest
        ? Number((navPerShare - latest.navPerShare).toFixed(2))
        : undefined,
      changePercent:
        latest && latest.navPerShare > 0
          ? Number(
              (
                ((navPerShare - latest.navPerShare) / latest.navPerShare) *
                100
              ).toFixed(2),
            )
          : undefined,
  ...
  const payload = await requestJson<NAVCalculation>(
