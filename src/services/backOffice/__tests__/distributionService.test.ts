...
const loadService = async () =>
  import("@/services/backOffice/distributionService");
...
describe("distributionService", () => {
  ...
  it("fetchDistributions filters by fundId", async () => {
    ...
    expect(
      results.every((distribution) => distribution.fundId === "fund-2"),
    ).toBe(true);
  });
  ...
  it("createDistribution adds a new distribution", async () => {
    ...
    expect(updated.some((distribution) => distribution.id === created.id)).toBe(
      true,
    );
  });
  ...
  it("updateDistribution updates fields and increments revision", async () => {
    ...
    const created = await createDistribution(buildDistribution());
    const updated = await updateDistribution(created.id, {
  ...
  it("deleteDistribution removes draft distributions", async () => {
    const { createDistribution, deleteDistribution, fetchDistributions } =
      await loadService();
    const created = await createDistribution(buildDistribution());
    await deleteDistribution(created.id);
    const results = await fetchDistributions();
    expect(results.some((distribution) => distribution.id === created.id)).toBe(
      false,
    );
  });
  ...
  it("deleteDistribution throws for non-draft distributions", async () => {
    ...
    await expect(deleteDistribution("dist-1")).rejects.toThrow(
  ...
  it("fetchDistributionCalendarEvents filters by date range", async () => {
    ...
    expect(
      events.every((event) => event.date >= startDate && event.date <= endDate),
    ).toBe(true);
  });
  ...
  it("fetchFeeTemplates filters by fundId", async () => {
    ...
    expect(
      templates.every(
        (template) => !template.fundId || template.fundId === "fund-2",
      ),
    ).toBe(true);
  });
});
