...
export async function getFundExpenses(fundId?: string): Promise<FundExpense[]> {
  if (isMockMode("backOffice")) {
    const values = fundId
      ? expenseStore.filter((item) => item.fundId === fundId)
      : expenseStore;
    return clone(values);
  }

  if (!fundId) return [];
  const payload = await requestJson<FundExpense[]>(
...
export async function addFundExpense(
  ...
  const result = await requestJson<FundExpense>(
...
export async function approveFundExpense(
  id: string,
  approver: string,
): Promise<FundExpense> {
  if (isMockMode("backOffice")) {
