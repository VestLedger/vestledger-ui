...
const navOpsSlice = createSlice({
  ...
  reducers: {
    ...
    navCalculateRequested: (
      state,
      _action: PayloadAction<{ fundId: string; fundName: string }>,
    ) => {
      state.calculations.status = "loading";
      state.calculations.error = undefined;
    },
    navReviewRequested: (
      state,
      _action: PayloadAction<{ calculationId: string; reviewedBy: string }>,
    ) => {
      state.calculations.status = "loading";
      state.calculations.error = undefined;
    },
    navPublishRequested: (
      state,
      _action: PayloadAction<{ calculationId: string; publishedBy: string }>,
    ) => {
      state.calculations.status = "loading";
      state.calculations.error = undefined;
    },
...
export const navOpsSelectors = createAsyncSelectors<NAVOpsData>(
  (state) => state.navOps.calculations,
);
