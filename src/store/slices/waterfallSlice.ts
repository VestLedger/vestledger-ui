...
const waterfallSlice = createSlice({
  ...
  reducers: {
    ...
    scenariosRequested: (
      state,
      _action: PayloadAction<GetWaterfallScenariosParams | undefined>,
    ) => {
    ...
    scenarioUpdated: (state, action: PayloadAction<WaterfallScenario>) => {
      if (state.scenarios.data?.scenarios) {
        const index = state.scenarios.data.scenarios.findIndex(
    ...
    createScenarioSucceeded: (
      state,
      action: PayloadAction<WaterfallScenario>,
    ) => {
    ...
    calculateWaterfallSucceeded: (
      state,
      action: PayloadAction<WaterfallCalculationData>,
    ) => {
    ...
    sensitivityAnalysisSucceeded: (
      state,
      action: PayloadAction<SensitivityAnalysisData>,
    ) => {
...
export const calculationSelectors =
  createAsyncSelectors<WaterfallCalculationData>(
    (state) => state.waterfall.calculation,
  );

export const sensitivitySelectors =
  createAsyncSelectors<SensitivityAnalysisData>(
    (state) => state.waterfall.sensitivityAnalysis,
  );

// Custom selectors
export const waterfallUISelectors = {
  selectSelectedScenarioId: (state: RootState) =>
    state.waterfall?.selectedScenarioId || null,
  ...
  selectComparisonScenarioIds: (state: RootState) =>
    state.waterfall?.comparisonScenarioIds || [],
