...
const analystPayload: AnalystDashboardData = {
  metrics: sharedMetric,
  recentDeals: [
    { name: "Acme AI", stage: "Due Diligence", score: 88, sector: "AI" },
  ],
...
const auditorPayload: AuditorDashboardData = {
  ...
  complianceItems: [
    { item: "AML Review", status: "Completed", lastCheck: "2026-01-04" },
  ],
};
...
const irPayload: IRDashboardData = {
  ...
  upcomingTasks: [
    { task: "Send monthly update", due: "Tomorrow", priority: "Medium" },
  ],
};
...
const researcherPayload: ResearcherDashboardData = {
  ...
  trendingTopics: [
    { topic: "Agentic tooling", sentiment: "Bullish", change: "+18%" },
  ],
};
...
describe("dashboardsSlice", () => {
  ...
  for (const suite of cases) {
    it(`${suite.key} handles request/success/failure and selectors`, () => {
      ...
      expect(
        (state as Record<string, { status: string }>)[suite.key].status,
      ).toBe("loading");
