...
describe("copilotSlice", () => {
  ...
  it("handles async suggestions lifecycle and selectors", () => {
    let state = copilotReducer(
      undefined,
      request("copilot/copilotSuggestionsRequested", { pathname: "/pipeline" }),
    );
    expect(state.suggestionsState.status).toBe("loading");

    state = copilotReducer(
      state,
      copilotSuggestionsLoaded({ suggestions, quickActions }),
    );
    const root = asRootState(state);
    expect(copilotSuggestionsSelectors.selectData(root)).toEqual({
      suggestions,
      quickActions,
    });
    ...
    expect(copilotSuggestionsSelectors.selectState(root)).toEqual(
    ...
    expect(copilotSuggestionsSelectors.selectError(failedRoot)).toEqual(
      testError,
    );
