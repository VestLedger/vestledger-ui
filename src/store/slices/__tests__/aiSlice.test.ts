...
describe("aiSlice", () => {
  ...
  it("handles pitch deck lifecycle and selectors", () => {
    let state = aiReducer(
      undefined,
      request("ai/pitchDeckAnalysesRequested", { dealId: "deal-1" }),
    );
    expect(state.pitchDeckState.status).toBe("loading");

    state = aiReducer(
      state,
      pitchDeckAnalysesLoaded({ analyses: mockAnalyses.slice(0, 2) }),
    );
    const root = asRootState(state);
    expect(pitchDeckSelectors.selectData(root)).toEqual({
    ...
    expect(pitchDeckSelectors.selectError(asRootState(state))).toEqual(
      testError,
    );
  });
  ...
  it("handles DD chat lifecycle and selectors", () => {
    let state = aiReducer(
      undefined,
      request("ai/ddChatConversationRequested", { dealId: 42 }),
    );
    ...
    state = aiReducer(
      ...
      ddChatConversationLoaded({
        dealId: 42,
        messages: ddMessages.slice(0, 1),
      }),
    );
    expect(state.ddChatState.data?.conversations["42"]).toEqual(
      ddMessages.slice(0, 1),
    );

    state = aiReducer(
      state,
      ddChatConversationLoaded({ dealId: 42, messages: ddMessages }),
    );
    ...
    expect(ddChatSelectors.selectConversation("42")(root)).toEqual([
      ...ddMessages,
      localMessage,
    ]);
