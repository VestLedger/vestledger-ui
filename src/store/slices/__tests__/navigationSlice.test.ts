...
describe("navigationSlice", () => {
  ...
  it("hydrates navigation and keeps core group expanded", () => {
    ...
    expect(state.sidebarState).toEqual({
      leftCollapsed: true,
  ...
  it("sets and toggles expanded groups correctly", () => {
    let state = navigationReducer(
      undefined,
      setExpandedGroups(["utilities", "core-operations"]),
    );
  ...
  it("updates and removes badges", () => {
    ...
    state = navigationReducer(
      state,
      updateBadge({ itemId: "notifications", badge: null }),
    );
  ...
  it("selector suite resolves expected values", () => {
    ...
    expect(navigationSelectors.selectExpandedGroups(root)).toEqual([
      "core-operations",
      "deal-management",
    ]);
    expect(navigationSelectors.selectBadges(root)).toEqual({});
    expect(navigationSelectors.selectSidebarState(root)).toEqual({
      leftCollapsed: true,
      rightCollapsed: false,
    });
