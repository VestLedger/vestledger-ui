...
describe('documentsSlice', () => {
  it('increments folder counts when adding a document', () => {
    const loadedState = documentsReducer(undefined, documentsLoaded(baseDocumentsData));
    ...
    expect(state.data?.folders.find((folder) => folder.id === 'folder-1')?.documentCount).toBe(1);
    expect(state.data?.folders.find((folder) => folder.id === 'folder-2')?.documentCount).toBe(1);
  });
  ...
  it('moves folder counts when a document changes folders', () => {
    ...
    expect(state.data?.folders.find((folder) => folder.id === 'folder-1')?.documentCount).toBe(0);
    expect(state.data?.folders.find((folder) => folder.id === 'folder-2')?.documentCount).toBe(1);
  });
