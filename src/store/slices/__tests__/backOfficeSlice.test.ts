...
describe("backOfficeSlice", () => {
  ...
  it("handles compliance lifecycle and selectors", () => {
    let state = backOfficeReducer(
      undefined,
      request("backOffice/complianceRequested"),
    );
    ...
    expect(complianceSelectors.selectError(asRootState(state))).toEqual(
      testError,
    );
  });
  ...
  it("handles fund admin lifecycle, upserts, and selectors", () => {
    let state = backOfficeReducer(
      undefined,
      request("backOffice/fundAdminRequested", { fundId: "fund-1" }),
    );
    expect(state.fundAdmin.status).toBe("loading");

    state = backOfficeReducer(state, fundAdminLoaded(fundAdminPayload));
    expect(
      fundAdminSelectors.selectData(asRootState(state))?.capitalCalls,
    ).toHaveLength(2);

    state = backOfficeReducer(
      state,
      request("backOffice/capitalCallCreateRequested", {
        fundId: "fund-2",
        fundName: "Fund II",
    ...
    state = backOfficeReducer(
      state,
      capitalCallCreateSucceeded(mockCapitalCalls[0]),
    );
    state = backOfficeReducer(
      state,
      capitalCallCreateSucceeded({
    ...
    expect(state.fundAdmin.data?.capitalCalls[0].purpose).toBe(
      "Updated purpose",
    );

    state = backOfficeReducer(
      state,
      request("backOffice/capitalCallUpdateRequested", {
    ...
    state = backOfficeReducer(
      state,
      request("backOffice/capitalCallSendRequested", {
        capitalCallId: mockCapitalCalls[0].id,
      }),
    );
    state = backOfficeReducer(
      state,
      request("backOffice/capitalCallReminderRequested", {
        capitalCallId: mockCapitalCalls[0].id,
      }),
    );
    state = backOfficeReducer(
      state,
      request("backOffice/lpReminderRequested", {
        lpResponseId: mockLPResponses[0].id,
      }),
    );
    state = backOfficeReducer(
      state,
      request("backOffice/lpResponseUpdateRequested", {
        lpResponseId: mockLPResponses[0].id,
    ...
    state = backOfficeReducer(
      state,
      request("backOffice/fundAdminExportRequested"),
    );
    state = backOfficeReducer(
      state,
      fundAdminExportSucceeded({ exportedAt: "2026-02-14T00:00:00.000Z" }),
    );
  ...
  it("handles tax center lifecycle and selectors", () => {
    let state = backOfficeReducer(
      undefined,
      request("backOffice/taxCenterRequested"),
    );
    ...
    expect(taxCenterSelectors.selectError(asRootState(state))).toEqual(
      testError,
    );
  });
  ...
  it("handles valuation lifecycle and selectors", () => {
    let state = backOfficeReducer(
      undefined,
      request("backOffice/valuation409aRequested"),
    );
    ...
    expect(valuation409aSelectors.selectState(root)).toEqual(
      state.valuation409a,
    );

    state = backOfficeReducer(state, valuation409aFailed(testError));
    expect(valuation409aSelectors.selectError(asRootState(state))).toEqual(
      testError,
    );
  });
  ...
  it("exposes failure actions for fund admin workflows", () => {
    let state = backOfficeReducer(
      undefined,
      capitalCallCreateFailed(testError),
    );
