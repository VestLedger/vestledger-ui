...
const secondaryTransferOpsSlice = createSlice({
  ...
  reducers: {
    ...
    secondaryTransfersLoaded: (
      state,
      action: PayloadAction<SecondaryTransferOpsData>,
    ) => {
    ...
    secondaryTransferReviewRequested: (
      state,
      _action: PayloadAction<{ transferId: string }>,
    ) => {
      state.data.status = "loading";
      state.data.error = undefined;
    },
    ...
    secondaryTransferROFRAdded: (
      state,
      action: PayloadAction<ROFRExercise>,
    ) => {
      if (!state.data.data) {
        state.data.data = { transfers: [], rofrExercises: [] };
      }
      const existing = state.data.data.rofrExercises.find(
...
export const secondaryTransferOpsSelectors =
  createAsyncSelectors<SecondaryTransferOpsData>(
