...
const distributionSlice = createSlice({
  ...
  reducers: {
    ...
    distributionsRequested: (
      state,
      _action: PayloadAction<DistributionFilters | undefined>,
    ) => {
    ...
    updateDistributionSucceeded: (
      ...
      if (state.distributions.data?.distributions) {
        ...
        if (index !== -1) {
          state.distributions.data.distributions[index] =
    ...
    deleteDistributionSucceeded: (state, action: PayloadAction<string>) => {
      if (state.distributions.data?.distributions) {
        state.distributions.data.distributions =
          state.distributions.data.distributions.filter(
    ...
    calendarEventsLoaded: (
      state,
      action: PayloadAction<CalendarEventsData>,
    ) => {
    ...
    statementTemplatesLoaded: (
      state,
      action: PayloadAction<StatementTemplatesData>,
    ) => {
...
export const statementTemplatesSelectors =
  createAsyncSelectors<StatementTemplatesData>(
...
export const distributionUISelectors = {
  selectSelectedDistributionId: (state: RootState) =>
    state.distribution?.selectedDistributionId || null,
  selectSelectedDistribution: (state: RootState): Distribution | null => {
    ...
    const distributions =
      state.distribution?.distributions.data?.distributions || [];
    return distributions.find((d) => d.id === distributionId) || null;
  },
  selectCurrentFilters: (state: RootState) =>
    state.distribution?.currentFilters || null,
