...
function readFundPreferences() {
  ...
  return {
    selectedFundId,
    viewMode,
    archivedFundIds: parseArchivedFundIds(safeLocalStorage.getItem(STORAGE_ARCHIVED_FUND_IDS)),
  };
}

export function DashboardRuntime() {
  const dispatch = useAppDispatch();
  const navigationHydratedState = useAppSelector(navigationSelectors.selectHydrated);
  const expandedGroups = useAppSelector(navigationSelectors.selectExpandedGroups);
  ...
  useEffect(() => {
    ...
    dispatch(
      navigationHydrated({
        expandedGroups: safeLocalStorage.getJSON<string[]>(NAVIGATION_STORAGE_KEY) ?? [],
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timeoutId);
  }, [navigationHydratedState, sidebarState.leftCollapsed, sidebarState.rightCollapsed]);
  ...
  useEffect(() => {
    ...
    latestFundsRequestRef.current?.abort?.();
    latestFundsRequestRef.current = dispatch(loadFundsOperation({})) as AbortableDispatch;
