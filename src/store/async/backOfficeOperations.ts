...
export const loadComplianceOperation = createLatestOperation<
  void,
  ComplianceData
>({
  ...
  run: async () => {
    const [complianceItems, regulatoryFilings, auditSchedule] =
      await Promise.all([
        getComplianceItems(),
        getRegulatoryFilings(),
        getAuditSchedule(),
...
export const createCapitalCallOperation = createLatestOperation<
  CapitalCallCreateInput,
  CapitalCall
>({
...
export const sendCapitalCallOperation = createLatestOperation<
...
export const sendLPReminderOperation = createLatestOperation<
  { lpResponseId: string },
  Awaited<ReturnType<typeof sendLPReminder>>
>({
...
>({
  ...
  run: async ({ arg }) =>
    recordLPResponsePayment(arg.lpResponseId, arg.amountPaid),
  onSuccess: (result) => lpResponseUpdateSucceeded(result),
  onFailure: (error) => lpResponseUpdateFailed(error),
});

export const exportFundAdminActivityOperation = createLatestOperation<
  void,
  { exportedAt: string }
>({
...
export const loadTaxCenterOperation = createLatestOperation<
  void,
  TaxCenterData
>({
  ...
  run: async () => {
    const [filingDeadline, taxDocuments, taxSummaries, portfolioTax] =
      await Promise.all([
        getTaxFilingDeadline(),
        getTaxDocuments(),
        getTaxSummaries(),
...
export const loadValuation409aOperation = createLatestOperation<
  void,
  Valuation409aData
>({
