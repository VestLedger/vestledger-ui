...
export function FeeExpenseTable({
  ...
  const handleUpdate = useCallback(
    (id: string, patch: Partial<FeeLineItem>) => {
      onChange(
        items.map((item) =>
          item.id === id
  ...
  const columns = useMemo<ColumnDef<FeeLineItem>[]>(() => [
    {
      key: 'type',
      label: 'Type',
      headerTitle: 'Fee category applied to the distribution.',
      width: '140px',
      render: (item) => (
        <Select
    ...
    },
    {
      key: 'description',
      label: 'Description',
      headerTitle: 'Line item description for reporting.',
      render: (item) => (
        <Input
    ...
    },
    {
      key: 'calculated',
      label: 'Calculated',
      headerTitle: 'Calculated total based on amount or percentage.',
      align: 'right',
      width: '120px',
      render: (item) => (
    ...
    },
    {
      key: 'notes',
      label: 'Notes',
      headerTitle: 'Notes for internal tracking.',
      render: (item) => (
        <Textarea
          value={item.notes ?? ''}
  ...
  return (
    <div className="space-y-4">
      ...
      <AdvancedTable
        ...
        pageSize={Math.max(items.length, 6)}
        emptyMessage="Add fee line items to build the distribution expense stack."
      />

      <div className="grid grid-cols-[140px_1fr_120px_120px_120px_1fr_40px] gap-2 rounded-lg border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-3 text-xs">
