...
describe('topbar search controller', () => {
  ...
  it('ignores stale results after a newer query is issued', async () => {
    vi.useFakeTimers();
    const dispatch = vi.fn();
    const alpha = deferred<Array<{ id: string; title: string; type: 'action' }>>();
    const beta = deferred<Array<{ id: string; title: string; type: 'action' }>>();
    const search = vi
      .fn()
    ...
    expect(dispatch).not.toHaveBeenCalledWith(
      searchLoaded({ results: [{ id: 'alpha', title: 'Alpha', type: 'action' }], query: 'alpha' })
    );
    ...
    expect(dispatch).toHaveBeenCalledWith(
      searchLoaded({ results: [{ id: 'beta', title: 'Beta', type: 'action' }], query: 'beta' })
    );
    vi.useRealTimers();
  });
  ...
  it('drops in-flight results after the query is cleared', async () => {
    vi.useFakeTimers();
    const dispatch = vi.fn();
    const pending = deferred<Array<{ id: string; title: string; type: 'action' }>>();
    ...
    pending.resolve([{ id: 'fund-admin', title: 'Fund Admin', type: 'action' }]);
    await Promise.resolve();

    expect(dispatch).toHaveBeenCalledWith(searchCleared());
    expect(dispatch).not.toHaveBeenCalledWith(
      searchLoaded({ results: [{ id: 'fund-admin', title: 'Fund Admin', type: 'action' }], query: 'fund admin' })
    );
    vi.useRealTimers();
  });
