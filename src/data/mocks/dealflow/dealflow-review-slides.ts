...
const sectorMarketData: Record<
  string,
  { tam: string; sam: string; som: string; growth: string }
> = {
...
const sectorDifferentiation: Record<string, string> = {
  "AI/ML":
    "Proprietary quantum-accelerated models deliver 10x faster inference at lower compute cost.",
  HealthTech:
    "Clinical-grade accuracy with seamless EMR integration and automated care workflows.",
  CleanTech:
    "Grid-aware charging network optimized for renewable-heavy regions and peak demand reduction.",
  Cybersecurity:
    "Zero-trust platform that reduces alert fatigue with AI-assisted threat triage.",
  FoodTech:
    "Demand-driven fulfillment that cuts food waste while improving on-time delivery.",
  FinTech:
    "Unified payments and treasury stack with instant settlement and global compliance coverage.",
  SaaS: "No-code customization and enterprise SSO reduce deployment time from months to weeks.",
};

const sectorProductDescriptions: Record<
  string,
  { description: string; differentiators: string[]; techStack: string[] }
> = {
  "AI/ML": {
  ...
  CleanTech: {
    description:
      "Sustainable energy infrastructure with smart grid integration",
    differentiators: [
      "Ultra-fast charging",
  ...
  const teamTemplates = [
    {
      ...
      backgrounds: [
        "MIT PhD, ex-Google",
        "Stanford PhD, ex-Meta",
        "Carnegie Mellon MS, ex-Amazon",
      ],
    },
    {
      role: "VP Sales",
      backgrounds: [
        "ex-Salesforce, 12yr enterprise",
        "ex-Oracle, 15yr B2B",
        "ex-SAP, 10yr SaaS",
      ],
    },
    {
      role: "VP Engineering",
      backgrounds: [
        "ex-Netflix, scaled 10x",
        "ex-Uber, built core systems",
        "ex-Stripe, payments expert",
      ],
    },
  ];
...
export function getMockDealflowReviewSlides(deal: Deal): DealflowReviewSlide[] {
  const marketData = sectorMarketData[deal.sector] || {
    tam: "$50B",
    sam: "$15B",
    som: "$2B",
    growth: "25% CAGR",
  };
  const competitors = sectorCompetitors[deal.sector] || [
    "Competitor A",
    "Competitor B",
    "Competitor C",
  ];
  ...
  const differentiation =
    sectorDifferentiation[deal.sector] ||
  ...
  return [
    ...
    },
    {
      ...
      content: {
        ...
        team: [
          {
            name: deal.founderName,
            role: "CEO & Founder",
            background: `${deal.sector} industry veteran`,
          },
          ...team,
        ],
        advisors: [
          `Prof. ${deal.sector} Expert (Stanford)`,
          `Dr. Industry Leader (MIT)`,
        ],
      },
    },
    {
