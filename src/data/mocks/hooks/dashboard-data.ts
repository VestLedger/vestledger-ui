...
const getDaysUntil = (target: Date, anchor: Date) =>
  Math.floor((target.getTime() - anchor.getTime()) / MS_PER_DAY);
...
const getLaneFromDomain = (domain?: Task["domain"]): BusinessLane => {
  ...
  if (domain === "capital-calls" || domain === "reporting")
    return "LP Relations";
  return "Operations";
};
...
export function getMockDashboardData(
  ...
  const alerts: Alert[] = [
    {
      ...
      description:
        "BioTech Inc has 6 months of runway remaining. GP decision needed on bridge structure and partner signal.",
      priority: 95,
      reasoning:
      ...
      prediction: {
        ...
        value: new Date(today.getTime() + 180 * MS_PER_DAY).toLocaleDateString(
          "en-US",
          {
            month: "short",
            day: "numeric",
    ...
    },
    {
      ...
      description:
        "FinServe now contributes outsized ARR share. Review concentration risk against return profile.",
      priority: 72,
      reasoning:
    ...
    },
    {
      ...
      description:
        "Acme Ventures and Beta Capital are 7 days overdue, creating drag on trust and close confidence.",
      priority: 68,
      reasoning:
  ...
  const tasks: Task[] = [
    {
      ...
      description:
        "Call Acme Ventures and Beta Capital to close overdue commitments.",
      ...
      delegationSuggestion: {
        ...
        reasoning:
          "Has direct LP relationship context and call cadence history.",
      },
    },
    {
      id: "task-2",
      ...
      description:
        "Decide term structure and partner signal for 6-month runway bridge.",
      domain: "portfolio",
      urgency: 10,
      impact: 10,
      priorityScore: 100,
    ...
    },
    {
      ...
      description:
        "Assess allocation fit, markup potential, and ownership strategy.",
      domain: "portfolio",
      urgency: 7,
      impact: 9,
      priorityScore: 63,
    ...
    },
    {
      ...
      description:
        "Align reporting quality and transparency points before LP circulation.",
      domain: "reporting",
      urgency: 8,
      impact: 7,
      priorityScore: 56,
    ...
    },
    {
      ...
      description:
        "Close remaining checklist items for annual compliance review.",
      domain: "compliance",
      urgency: 6,
      impact: 6,
      priorityScore: 36,
  ...
  const revenueByCompanyId: Record<
    string,
    {
      arr: number;
      arrGrowthQoq: number;
  ...
  > = {
    ...
    "pc-2": {
      ...
      valuation: 140,
      potential: "watch",
      upsideLabel: "Bridge preserves option value",
    },
    "pc-3": {
      arr: 79,
      arrGrowthQoq: 42,
  ...
  const taskItems: DailyBriefItem[] = tasks
    ...
    .flatMap((task) => {
      const dueDate = addDays(
        today,
        taskDueDateOffsets[task.id] ?? BRIEF_HORIZON_DAYS,
      );
      ...
      return [
        {
          ...
          searchHint:
            task.domain === "portfolio"
              ? task.title
                  .replace("Co-invest review for ", "")
                  .replace("Bridge extension decision for ", "")
              : undefined,
          quadrant: getPriorityQuadrant(
            task.urgency,
            task.impact,
            dueDate,
            today,
  ...
  const warningItems: DailyBriefItem[] = alerts
    ...
    .flatMap((alert) => {
      const dueDate = addDays(
        today,
        alertDueDateOffsets[alert.id] ?? BRIEF_HORIZON_DAYS,
      );
      if (!isWithinBriefWindow(dueDate, today)) return [];
  ...
  const dailyBriefItems = [
    ...taskItems,
    ...warningItems,
    ...overdueCapitalCallWarnings,
  ].sort((a, b) => {
    const quadrantDiff =
      getQuadrantRank(a.quadrant) - getQuadrantRank(b.quadrant);
  ...
  const blockers: HomeBlocker[] = dailyBriefItems
    ...
    .map((item) => {
      ...
      return {
        ...
        blockedDays:
          blockerAgeByBriefId[item.id] ??
          Math.max(1, Math.abs(getDaysUntil(item.dueDate, today)) + 1),
        severity,
        lane: item.lane,
        route: item.route,
    ...
    .sort((a, b) => {
      const severityDiff =
        blockerSeverityRank[a.severity] - blockerSeverityRank[b.severity];
  ...
  const topRevenueSum = topRevenueContributors.reduce(
    (sum, row) => sum + row.arr,
    0,
  );
  const currentPortfolioArr =
    portfolioRevenueTrend[portfolioRevenueTrend.length - 1]?.arr ??
    topRevenueSum;
  ...
  const metrics = {
    ...
    atRiskCompanies: portfolioRevenueRows.filter(
      (c) => c.riskFlag === "critical",
    ).length,
    healthyCompanies: portfolioRevenueRows.filter(
      (c) => c.riskFlag === "stable",
    ).length,
    totalTasks: tasks.filter((t) => t.status !== "completed").length,
    urgentTasks: tasks.filter(
      (t) => t.priorityScore >= 70 && t.status !== "completed",
    ).length,
  };
