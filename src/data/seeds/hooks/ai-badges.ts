...
const calculateFundAdminBadge = (
  ...
  const totalOverdueAmount = overdueCalls.reduce(
  ...
  const avgDaysOverdue =
    overdueCalls.reduce((sum, call) => {
      const daysOverdue = Math.floor(
        (MOCK_NOW.getTime() - call.dueDate.getTime()) / (24 * 60 * 60 * 1000),
      );
...
const calculateComplianceBadge = (
  tasks: ComplianceTask[],
): NavigationBadge | null => {
  const today = new Date(MOCK_NOW.getTime());
  const upcomingTasks = tasks.filter((task) => {
    const daysUntilDeadline =
      (task.deadline.getTime() - today.getTime()) / (24 * 60 * 60 * 1000);
    return (
      daysUntilDeadline <= 7 &&
      daysUntilDeadline > 0 &&
  ...
  const highComplexityCount = upcomingTasks.filter(
    (t) => t.complexity === "high",
  ).length;
  const nearestDeadline = Math.min(
    ...upcomingTasks.map((t) => t.deadline.getTime()),
  );
  const daysUntil = Math.ceil(
    (nearestDeadline - today.getTime()) / (24 * 60 * 60 * 1000),
  );
...
const calculateLPManagementBadge = (
  capitalCalls: CapitalCall[],
): NavigationBadge | null => {
...
const calculate409ABadge = (): NavigationBadge | null => {
  const today = new Date(MOCK_NOW.getTime());
  const lastValuation = new Date(
    today.getTime() - 11 * 30 * 24 * 60 * 60 * 1000,
  ); // 11 months ago
  const monthsSinceValuation =
    (today.getTime() - lastValuation.getTime()) / (30 * 24 * 60 * 60 * 1000);
  ...
  return {
    ...
    tooltip:
      "409A valuation due for renewal - AI detected based on 12-month cycle",
  };
};

const calculateAnalyticsBadge = (
  companies: PortfolioCompany[],
): NavigationBadge | null => {
