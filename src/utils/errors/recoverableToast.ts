...
function emitToastEvent(payload: RecoverableToastEventPayload): void {
  ...
  window.dispatchEvent(new CustomEvent<RecoverableToastEventPayload>(RECOVERABLE_TOAST_EVENT, { detail: payload }));
}
...
export function emitRecoverableErrorToast(
  ...
  const message = error.message?.trim() || options?.fallbackMessage || 'Unable to load the latest data.';
