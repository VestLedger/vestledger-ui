...
import type {
  AIInsight,
  InsightDetail,
} from "@/components/dashboard/ai-insights-banner";
...
export function useAIInsights(data: DashboardData): AIInsight {
  return useMemo(() => {
    ...
    if (data.overdueCapitalCalls > 0) {
      insights.push(
        `${data.overdueCapitalCalls} capital call${data.overdueCapitalCalls > 1 ? "s" : ""} overdue`,
      );
      details.push({
        id: "capital-calls",
        category: "urgent",
        ...
        actionable: false,
        reasoning:
          "Historical data shows that capital calls overdue by more than 5 days have a 40% higher chance of requiring additional follow-up. Early intervention improves collection rates by 25%.",
      });
    }
    ...
    if (data.atRiskCompanies > 0) {
      insights.push(
        `${data.atRiskCompanies} compan${data.atRiskCompanies > 1 ? "ies" : "y"} at risk`,
      );
      details.push({
        id: "portfolio-risk",
    ...
    if (data.upcomingDeadlines > 0) {
      insights.push(
        `${data.upcomingDeadlines} compliance deadline${data.upcomingDeadlines > 1 ? "s" : ""} approaching`,
      );
      details.push({
        id: "compliance",
        category: "warning",
        ...
        actionable: false,
        reasoning:
          "Based on historical task complexity scores and team bandwidth, these deadlines require proactive planning to avoid last-minute rushes.",
      });
    }
    ...
    if (data.healthyCompanies > 0 && data.atRiskCompanies === 0) {
      details.push({
        ...
        actionable: false,
        reasoning:
          "Health scores above 80 and runway >12 months indicate stable operations. Continue monitoring quarterly for early warning signs.",
      });
    }
    ...
    if (data.overdueCapitalCalls === 0 && data.upcomingDeadlines === 0) {
      details.push({
        ...
        description:
          "No overdue items or immediate compliance concerns detected. All capital calls on track.",
        actionable: false,
        reasoning:
          "Current metrics indicate well-managed fund operations with strong LP relationships and timely compliance.",
      });
    }
    ...
    if (insights.length === 0) {
      summary =
        "No urgent actions detected. Fund operations and compliance workflows are on track.";
