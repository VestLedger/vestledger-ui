...
function createMessage(type: CopilotMessage['type'], content: string): CopilotMessage {
  return {
    id: nextMessageId(),
    type,
    content,
...
export function openCopilotWithQuery(dispatch: AppDispatch, pathname: string, query: string) {
...
export function sendCopilotMessage(dispatch: AppDispatch, pathname: string, content: string) {
  return runCopilotInteraction({
    dispatch,
    userContent: content,
    clearInput: true,
...
export function invokeCopilotQuickAction(
  ...
  return runCopilotInteraction({
    ...
    userContent: label,
    delayMs: 800,
    buildResponse: async () => `I'm working on "${actionText}". This will take a moment...`,
    trackingContext: { context: 'invokeCopilotQuickAction', pathname, actionLabel: label },
  });
}
...
export function invokeCopilotSuggestion(
  ...
  return runCopilotInteraction({
    ...
    trackingContext: { context: 'invokeCopilotSuggestion', suggestionText: suggestion.text },
  });
}
